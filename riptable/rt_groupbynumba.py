...
class GroupbyNumba(GroupByOps):
    ...
    @nb.jit(parallel=True, nopython=True, cache=True)
    def _numbaEMA(
        iGroup, iFirstGroup, nCountGroup, binLow, binHigh, data, ret, time, decayRate
    ):
    ...
    def grpTrim(grp, x, y):
        ...
        for colName in grp._dataset:
            if colName not in grp.gbkeys.keys():
                ...
                grp._numbaTrim(
                    iGroup, iFirstGroup, nCountGroup, grp._dataset[colName], ret, x, y
                )
                retData[colName] = ret
        return retData
    ...
    def grpFillForward(grp):
        ...
        for colName in grp._dataset:
            if colName not in grp.gbkeys.keys():
                ret = empty(grp._dataset.shape[0])
                grp._numbaFillForward(
                    iGroup, iFirstGroup, nCountGroup, grp._dataset[colName], ret
                )
                retData[colName] = ret
        return retData
        ...
        for colName in grp._dataset:
            if colName not in grp.gbkeys.keys():
                ...
                grp._numbaFillForward(
                    ...
                    iFirstGroup,
                    nCountGroup,
                    grp._dataset[colName],
                    forwardFilled,
                )
                ret = empty(grp._dataset.shape[0])
                grp._numbaFillBackward(
                    iGroup, iFirstGroup, nCountGroup, forwardFilled, ret
                )
                retData[colName] = ret
        return retData
    ...
    def nb_ema(self, *args, time=None, decay_rate=None, **kwargs):
        ...
        return self._calculate_all(
            NUMBA_REVERSE_FUNC[GroupbyNumba.nb_ema],
            *args,
            func_param=(time, decay_rate),
            **kwargs,
        )
    ...
    def nb_sum(self, *args, **kwargs):
        """Compute sum of group"""
        return self._calculate_all(
            NUMBA_REVERSE_FUNC[GroupbyNumba.nb_sum], *args, **kwargs
        )
    ...
    def nb_min(self, *args, **kwargs):
        """Compute sum of group"""
        return self._calculate_all(
            NUMBA_REVERSE_FUNC[GroupbyNumba.nb_min], *args, **kwargs
        )
