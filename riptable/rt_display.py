...
__all__ = ["DisplayCell", "DisplayColumn", "DisplayText", "DisplayDetect", "DisplayString", "DisplayTable"]
...
from .rt_numpy import arange, bool_to_fancy, hstack, ismember
            ...
            try:
                ...
                if DisplayOptions.COLOR_MODE is not None:
                    ...
                    if color_detected == "Linux" or color_detected == "LightBG":
                        ...
                        "PYCHARM_HOSTED" in os.environ and os.environ["PYCHARM_HOSTED"] == "1"
                    ):
...
class DisplayText(object):
    ...
    @staticmethod
    def _as_if_dark():
        return DisplayDetect.ColorMode == DisplayColorMode.Dark and DisplayDetect.Mode != DisplayDetectModes.Jupyter
...
class DisplayTable(object):
    ...
    def build_result_table_new(
        ...
        if len(self._attribs) > 0 and self._column_sets is None:
            ...
            num_footer_rows = self._attribs.get(DisplayAttributes.NUMBER_OF_FOOTER_ROWS, 0)
            for i, name in enumerate(main_header_names):
                is_margin_column = name in self._attribs.get(DisplayAttributes.MARGIN_COLUMNS, [])
                if is_margin_column:
    ...
    def add_required_columns(
        self, header_names, table_data, footers, masked=False, gbkeys=None, transpose=False, color=None, style=None
    ):
        """
...
class DisplayHtmlTable:
    def __init__(
        self, headers, left_columns, main_columns, right_columns: Optional[list] = None, footers=None, axis_labels=False
    ):
...
class DisplayColumn:
    ...
    def build_footer(self, final_color=None, plain=False):
        ...
        if isinstance(self._footer, list):
            return [
                self._build_end(f, final_color=final_color, plain=plain, justification=DisplayJustification.Right)
                for f in self._footer
            ]
        return self._build_end(
            self._footer, final_color=final_color, plain=plain, justification=DisplayJustification.Right
        )
    ...
    def styled_string_list(self):
        ...
        if (
            self._color in (DisplayColumnColors.Multiset_head_a, DisplayColumnColors.Multiset_head_b)
        ...
        if self._max_t_widths is not None:
            for i, cell in enumerate(self._data):
                self._data[i] = self.align_console_string(
                    cell.string, self._max_t_widths[i], align=self._item_format.justification
                )
        else:
            for i, cell in enumerate(self._data):
                self._data[i] = self.align_console_string(
...
class DisplayCell:
    ...
    # ----------------------------------------------------------------------------
    def __init__(self, string, value=None, color=None, html=False, colspan=None, align=None):
        self.prefix = ""
        self.suffix = ""
        self.string = string
        self.value = value
