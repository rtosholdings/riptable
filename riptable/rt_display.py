            ...
            try:
                ...
                if DisplayOptions.COLOR_MODE is not None:
                    ...
                    if color_detected == "Linux" or color_detected == "LightBG":
                        ...
                        and os.environ["PYCHARM_HOSTED"] == "1"
                    ):
...
class DisplayText(object):
    ...
    def _as_if_dark():
        return (
            DisplayDetect.ColorMode == DisplayColorMode.Dark
            and DisplayDetect.Mode != DisplayDetectModes.Jupyter
        )
...
class DisplayTable(object):
    ...
    def __init__(self, attribs: Optional[dict] = None):
        ...
        if DisplayTable.FORCE_REPR is False:
            ...
            if self._console_x is None:
                if DisplayTable.DebugMode:
                    print("could not detect console size. using defaults.")
        ...
        # check for html
        if (
            self._display_mode == DisplayDetectModes.HTML
            or self._display_mode == DisplayDetectModes.Jupyter
        ):
    ...
    def build_result_table_new(
        ...
        # MAIN TABLE INFO
        self._header_tups = (
            header_tups  # list of lists of header tuples (see ColHeader in rt_enum)
        )
        ...
        self._gb_prefix = (
            TypeRegister.DisplayOptions.GB_PREFIX
        )  # add star before groupby names
        self._footer_name = ""
        ...
        self._missing_sort_cols = (
            False  # sort column names were provided, but not found in the data
        )

        # COLUMN STYLES
        self._row_numbers = row_numbers
        number_style = ColumnStyle(
            color=DisplayColumnColors.Rownum, align=DisplayJustification.Right
        )
        ...
        if self._transpose_on:
            ...
            if DisplayTable.DebugMode:
                print("*table untransposed")
            totalrows = nrows
            # force all rows to be shown
            if self.options.ROW_ALL is True:
                if DisplayTable.DebugMode is True:
                    print("*forcing all rows")
                head = nrows
                tail = 0
        ...
        # untransposed
            ...
            if self._gbkeys is not None:
                ...
                for i in range(nKeys):
                    ...
                    left_header_names.append(
                        self._gb_prefix + str(self._header_tups[-1][i][0])
                    )
        ...
        if self._footer_tups is not None:
            left_footers = [
                frow[: len(left_header_names)] for frow in self._footer_tups
            ]
        ...
        self._left_table_columns, self._left_widths = self.add_required_columns(
            left_header_names,
        ...
        if self._right_cols is not None:
            ...
            self._right_table_columns, self._right_widths = self.add_required_columns(
                right_header_names, self._right_table_data, right_footers
            )
        ...
        if self._transpose_on:
            ...
            if (
                self.options.COL_ALL
                and ((self._html_on is False) or self._from_str)
                and self._num_header_lines == 1
            ):
                ...
                (
                    self._main_table_columns,
                    self._main_first_widths,
                    self._main_last_widths,
                ) = self.fit_max_columns(
                    self._header_names,
                    self._main_data,
                    self._total_width,
                    self._console_x,
            ...
            if self._column_sets is None:
                if not self._missing_sort_cols:
                    for i, c in enumerate(self._col_sortlist):
                        self._main_table_columns[i].paint_column(
                            sort_style, badrows=badrows
                        )
            else:
                for i, c in enumerate(self._col_sortlist):
        ...
        if badcols is not None or badrows is not None:
            for i, col in enumerate(self._main_table_columns):
                ...
                if color_style is not None or badrows is not None:
                    self._main_table_columns[i].style_column(
                        color_style, badrows=badrows
                    )
        ...
        if len(self._attribs) > 0 and self._column_sets is None:
            ...
            for i, name in enumerate(main_header_names):
                ...
                if num_footer_rows:
                    self._main_table_columns[i].paint_column(
                        DisplayColumnColors.Groupby, slice(-num_footer_rows, None, None)
                    )

            # Right justify row label(s) of footer rows
            for i, name in enumerate(left_header_names):
                if num_footer_rows:
                    self._left_table_columns[i].align_column(
                        DisplayJustification.Right, slice(-num_footer_rows, None, None)
                    )
        ...
        if (
            ...
            and self._transpose_on is False
            and self._col_break is not None
        ):
            self.fix_multiline_headers()
            final_footers = self.fix_multiline_footers(
                plain=from_str, badcols=badcols, badrows=badrows
            )
        ...
        # all columns requested in console
            ...
            for idx, main_widths in enumerate(self._set_widths):
                self._set_widths[idx] = (
                    self._left_widths + main_widths + self._right_widths
                )

        final_headers, final_footers = self.build_final_ends(
            plain=from_str, badcols=badcols, badrows=badrows
        )
            ...
            if self._column_sets is None:
                ...
                for set_index, set in enumerate(self._header_sets):
                    for i, cell in enumerate(set[-1]):
                        color = DisplayColumnColors.Multiset_col_a + (
                            cell.color_group % 2
                        )
        ...
        left_table_strings = style_cells(
            self._left_table_columns, stylefunc, callback=callback, location="left"
        )
        if self._right_cols is not None:
            right_table_strings = style_cells(
                self._right_table_columns,
                stylefunc,
                callback=callback,
                location="right",
        ...
        for col_set in final_column_sets:
            col_set = style_cells(
                col_set, stylefunc, as_rows, callback=callback, location="main"
            )
        ...
        if self._column_sets is None:
            ...
            if self._html_on:
                result_table = DisplayHtmlTable(
                    final_headers,
                    left_table_strings,
                    main_table_strings,
                    right_columns=right_table_strings,
                    footers=final_footers,
                )
            else:
                result_table = DisplayConsoleTable(
                    self._all_widths,
                    final_headers,
                    left_table_strings,
                    main_table_strings,
    ...
    def build_final_headers_html(self, plain=False):
        ...
        for i, line in enumerate(self._header_tups):
            ...
            for j, cell in enumerate(line):
                ...
                new_cell = DisplayCell(
                    cell.col_name,
                    color=color,
                    html=self._html_on,
                    colspan=cell.cell_span,
                )
                new_cell.paint_cell()
        ...
        **specifically for multi-line
        ...
        for i, line in enumerate(self._header_tups[:-1]):
            ...
            for j, cell in enumerate(line):
                ...
                combined_width = sum(
                    self._all_widths[width_index : width_index + cell.cell_span]
                )
                margin_width = (cell.cell_span - 1) * column_margin
                combined_width += margin_width
                new_cell.string = DisplayColumn.align_console_string(
                    new_cell.string, combined_width, align=DisplayJustification.Center
                )
        ...
        bottom_headers = []
        bottom_colors = [
            DisplayColumnColors.Multiset_head_a + (cell.color_group % 2)
            for cell in self._header_tups[-1]
        ]
        for c in self._left_table_columns:
        ...
        for i, c in enumerate(self._main_table_columns):
            bottom_headers.append(
                c.build_header(
                    bottom_colors[i], plain=plain, align=DisplayJustification.Center
                )
            )
    ...
    def build_final_ends(self, plain=False, badcols=None, badrows=None):
        ...
        if self._transpose_on:
            ...
            for idx, width in enumerate(
                self._all_widths[len(self._left_table_columns) :]
            ):
                new_cell = DisplayCell(
                    str(idx),
                    color=DisplayColumnColors.Rownum,
                ...
                if self._html_on:  # and from_str is False
                    ...
                    new_cell.string = DisplayColumn.align_console_string(
                        new_cell.string, width, align=DisplayJustification.Right
                    )
            ...
            for c in reversed(self._left_table_columns):
                if self._from_str:
                    ...
                    transposed_headers.insert(
                        0, c.build_header(DisplayColumnColors.Rownum)
                    )
            ...
            # default to the headers constructed in DisplayColumns
                ...
                if self._column_sets is None:
                    ...
                    for c in (
                        ...
                        if (
                            badcols is not None
                            and badcols.get(c.header, None) is not None
                        ):
                            bottom_footers.append(
                                c.build_footer(final_color=gray_style, plain=plain)
                            )
                        else:
        ...
        for i, top_header in enumerate(top_header_row):
            ...
            if (col_idx + current_span) < num_left_cols - 1:
                ...
                # trim last span
                    ...
                    last_tup = ColHeader(
                        top_header.col_name, new_span, top_header.color_group
                    )
    ...
    def build_column(self, header, column, masked=False, footer=None, style=None):
        ...
        if self._row_break is not None:
            ...
            cell_list = [
                DisplayCell(func(item, display_format), item, html=self._html_on)
                for item in column[mask[: self._row_break]]
            ]
        ...
        new_column = DisplayColumn(
            cell_list,  # build a display cell for every value in the masked column
    ...
    def add_required_columns(
        ...
        for i, column in enumerate(table_data):
            ...
            new_column, _ = self.build_column(
                header, column, masked=masked, style=style, footer=footerval
            )
            ...
            if gbkeys is not None and transpose:
                new_column.paint_column(
                    DisplayColumnColors.Groupby, col_slice=slice(None, len(gbkeys))
                )
    ...
    # -------------------------------------------------------------------------------------
    def fit_max_columns(
        self, headers, columns, total_width, console_width, footers=None
    ):
        """
        ...
        *in progress
        ...
        # self._console_x -= 80
        while ((total_width <= console_width) or force_all_columns is True) and (
            colbegin <= colend
        ):
            # pull from the front
            c = columns[colbegin]
            ...
            if (
                (total_width + d_first_width) > console_width
            ) and force_all_columns is False:
            ...
            # front column fit
                ...
                if (
                    (total_width + d_last_width) > console_width
                ) and force_all_columns is False:
            ...
            if DisplayTable.DebugMode:
                print("console_x", console_width)
            if DisplayTable.DebugMode:
                print("colbegin", colbegin)
            if DisplayTable.DebugMode:
                print("colend", colend)
    ...
    # -------------------------------------------------------------------------------------
    def all_columns_console_multiline(
        self, console_width, left_offset, headers, columns
    ):
        """
        ** not implemented
        ...
        header_sets = [
            [top_headers[i], bottom_headers[i]] for i in range(len(top_headers))
        ]
...
class DisplayHtmlTable:
    def __init__(
        ...
        left_columns,
        main_columns,
        right_columns: Optional[list] = None,
        footers=None,
        axis_labels=False,
    ):
    ...
    def build_table(self):
        ...
        )  # unique id to fix bug in styling (jupyter lab likes to take over)
        ...
        for i, left_cells in enumerate(self.left_columns):
            html_string_list.append(
                f"<tr>{join_row_section(left_cells)}{join_row_section(self.main_columns, i)}{join_row_section(self.right_columns, i)}</tr>"
            )
...
class DisplayConsoleTable:
    ...
    def build_end(self, is_header=True):
        ...
        if is_header is True:
            end_cells = self.headers
        else:
            end_cells = self.footers
        ...
        border_row = DisplayConsoleTable.column_spacer.join(
            [DisplayConsoleTable.border_style * width for width in self.widths]
        )
        if is_header is True:
            end_str.append(border_row)
        else:
            end_str.insert(0, border_row)
    ...
    def build_data(self):
        ...
        for data in column_data:
            row = DisplayConsoleTable.column_spacer.join(
                [i for sub in data for i in sub]
            )
            data_str.append(row)
...
class DisplayColumn:
    ...
    def __init__(
        ...
        # footers is a list of strings
            ...
            self._max_width = max(
                self._max_width, len(header), len(max(footer, key=len))
            )
            self._footer = footer
    ...
    def build_header(self, final_color=None, plain=False, align=None):
        ...
        return self._build_end(
            self._header, final_color=final_color, plain=plain, justification=align
        )
    ...
    def _build_end(self, end, final_color=None, plain=False, justification=None):
        ...
        if self._html:
            ...
            if end is self._header:
                styled_end.prefix += " " + self.text_decoration_html(
                    DisplayTextDecoration.Bold
                )  # headers will always be bold
            styled_end.prefix += " " + self.align_html(justification)
        styled_end.string = self.align_console_string(
            styled_end.string, width, align=justification
        )
    ...
    def styled_string_list(self):
        ...
        if self._html is False:
            ...
            for i, cell in enumerate(self._data):
                ...
                cell.string = self.align_console_string(
                    cell.string, max_width, align=justification
                )
    ...
    def plain_string_list(self):
        if self._max_t_widths is not None:
            for i, cell in enumerate(self._data):
                self._data[i] = self.align_console_string(
                    cell.string,
                    self._max_t_widths[i],
                    align=self._item_format.justification,
                )
        else:
            for i, cell in enumerate(self._data):
                self._data[i] = self.align_console_string(
                    cell.string, self._max_width, align=self._item_format.justification
                )
...
class DisplayCell:
    ...
    # ----------------------------------------------------------------------------
    def __init__(
        self, string, value=None, color=None, html=False, colspan=None, align=None
    ):
        self.prefix = ""
        self.suffix = ""
        self.string = string
