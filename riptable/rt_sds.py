...
def sds_os(func, path):
    ...
    """
    if SDSVerbose:
        start = VerbosePrint(f"calling {func.__name__} on {path}")
    d = func(path)
    if SDSVerbose:
        VerbosePrintElapsed(f"finished {func.__name__}", start)
    return d
...
def sds_flatten(rootpath):
    ...
    for dirpath in nested:
        ...
        if skip:
            warnings.warn(
                f"{dirpath} contained items that were not .sds files or directories. Could not flatten"
            )
        ...
        if "_root.sds" in dirlist:
            ...
            if sds_exists(newroot):
                warnings.warn(
                    f"temp file {newroot} already in root container, could not flatten {dirpath}."
                )
                skip = True
            else:
        ...
        for fname in dirlist:
            ...
            if sds_exists(new):
                warnings.warn(
                    "{new} was already found in base directory. Could not flatten {dirpath}."
                )
                skip = True
                break
            else:
...
def _sds_save_single(
    ...
    """
    _, name, status = _sds_path_single(
        path, share=share, overwrite=overwrite, name=name, append=append
    )
    if status is False:
        return
...
def save_sds(
    ...
    if isinstance(item, TypeRegister.Dataset):
        ...
        _, _, status = _sds_path_single(
            filepath, share=share, overwrite=overwrite, name=name, append=append
        )
        if status is False:
            return
        _write_to_sds(
    ...
    # pack array into struct for save (will handle subclasses)
        ...
        raise TypeError(
            f"save_sds() can only save Structs, Datasets, or single arrays. Got {type(item)}"
        )
...
def _sds_raw_info(filepath, share=None, sections=None, threads=None) -> List[tuple]:
    ...
    if not sds_endswith(filepath):
        ...
        if sds_isdir(filepath):
            raise ValueError(
                f"The filename {filepath} is a directory and ends with .sds so sds_info will not work.  Consider sds_tree(filepath) instead."
            )

    return decompress_dataset_internal(
        filepath, info=True, sections=sections, threads=threads
    )
...
def load_sds_mem(
    ...
    return _load_sds_internal(
        filepath, share=share, include=include, threads=threads, filter=filter
    )
        ...
        if sds_isdir(f):
            ...
            if sds_isfile(fnew):
                ...
                if mustexist is True:
                    raise ValueError(
                        f"Could not find file named {f} and mustexist is True."
                    )
                else:
...
def _sds_load_from_list(
    ...
    *future optimization: reduce this to 1 call for all files (gets tricky with nested structures)
    ...
    multiload = decompress_dataset_internal(
        single_sds,
        sharename=share,
        info=info,
        include=include,
        ...
        for idx, f in enumerate(files):
            if f is None:
                files[idx] = _read_sds(
                    ...
                    info=info,
                    include=include,
                    multiload=multiload[single_idx],
                    filter=filter,
                    mustexist=mustexist,
                    sections=sections,
                )
...
# -----------------------------------------------------------------------------------------
def _multistack_categoricals(
    spec_name, meta_list, indices, listcats, idx_cutoffs, unique_cutoffs
):
    """
    ...
    if SDSVerbose:
        verbose_start = VerbosePrint(f"start reconstructing categorical {spec_name}")
    ...
    if mode in (CategoryMode.Dictionary, CategoryMode.IntEnum):
        ...
        if len(base_index) != 1:
            raise TypeError(
                f"Categoricals had different base index {base_index}, cannot be stacked!"
            )
    ...
    if SDSVerbose:
        VerbosePrintElapsed(
            f"finished reconstructing cateogorical {spec_name}", verbose_start
        )
    return result
...
def _multistack_onefile(arrays, nameflag_tup, cutoffs, meta, sep="/"):
    ...
    s = TypeRegister.Struct._flatten_undo(
        sep, 0, startname, obj_array, meta=meta, cutoffs=cutoffs
    )
        ...
        if itemenum & SDSFlag.OriginalContainer:
            ...
            if itemname in spec_meta:
                ...
                if not TypeRegister.is_binned_type(i_class):
                    underlying = i_class._load_from_sds_meta_data(
                        itemname, underlying, [], i_meta
                    )
    ...
    for spec_name, meta_list in spec_meta.items():
        ...
        if underlying is not None:
            ...
            stacked_categorical = _multistack_categoricals(
                spec_name, meta_list, underlying, listcats, idx_cutoffs, unique_cutoffs
            )
...
def _stacked(filenames, result, folders):
    ...
    if SDSVerbose:
        verbose_start = VerbosePrint(f"starting _multistack_items")
    ...
    if isonefile:
        data, allcutoffs = _multistack_onefile(
            arrays, meta_tups, cutoffs, meta, sep="/"
        )
    ...
    if SDSVerbose:
        VerbosePrintElapsed(f"finished _multistack_items", verbose_start)
    ds = TypeRegister.PDataset(data, cutoffs=cutoffs, filenames=loadedpaths)
    ...
    if isinstance(filter, np.ndarray) and filter.dtype.char != "O":
        if filter.dtype.char == "?" and len(filter) != 0:
            ...
            if filter.dtype.num > 10:
                raise TypeError(
                    f"The filter must be a numpy array of booleans or integers not {filter.dtype}."
                )
        ...
        raise TypeError(
            f"The filter must be a numpy array of booleans or integers not {type(filter)}."
        )


# -----------------------------------------------------------------------------------------
def _stack_sds_files(
    filenames,
    share=None,
    info=False,
    include=None,
    ...
    if threads is not None:
        savethreads = rc.SetThreadWakeUp(threads)
    try:
        if len(filenames) == 0:
            raise ValueError(
                f"MultiStack list was empty. No files existed in original list."
            )

        if SDSVerbose:
            verbose_start = VerbosePrint(
                f"calling rc.MultiStackFiles with {len(filenames)} files first: {filenames[0]} last: {filenames[-1]} include: {include}"
            )
        ...
        if result is None:
            raise ValueError(
                f"There was a problem when trying to stack the files {filenames}.  No data was returned."
            )
...
def _stack_sds_dirs(
    ...
    for inc, files in include_dict.items():
        include_dict[inc] = _stack_sds_files(
            files, share=share, info=info, sections=sections
        )
...
def _load_sds_internal(
    ...
    if folders is not None:
        ...
        if not isinstance(folders, list):
            raise ValueError(
                f"The folders kwarg must be a list of strings of dataset or struct names to include. {folders}"
            )
    ...
    if stack is True:
        ...
        files, sds_filelist, badlist, hasdir, hasfile = _sds_dir_from_file_list(
            filepath, share=share, mustexist=mustexist
        )

        if hasdir:
            if hasfile:
                raise TypeError(
                    f"List of files must contain only directories or only .sds files. {filepath}"
                )
            else:
                # only directories
                if include is None and folders is None:
                    raise ValueError(
                        f"SDS stacking only implemented for Datasets. Must provide folders list if loading from multiple Struct directories."
                    )
                return _stack_sds_dirs(
                    files,
                    share=share,
                    info=info,
                    include=include,
            ...
            if folders is not None:
                ...
                for f in folders:
                    fname = f[:-1]
                    include_dict[fname] = _stack_sds_files(
                        sds_filelist,
                        share=share,
                        info=info,
                        include=include,
    ...
    # list of full filepaths provided
        ...
        files, single_sds, _, _, _ = _sds_dir_from_file_list(
            filepath, mustexist=mustexist
        )
        return _sds_load_from_list(
            files,
            single_sds,
            share=share,
            info=info,
    ...
    if sds_endswith(filepath) or share is not None:
        ...
        if origerror is not None:
            ...
            try:
                ...
                raise ValueError(
                    f"Could not load item with filepath {filepath!r} and shared name {share!r}.  First error: {origerror!r}.  Second error {sys.exc_info()[1]}"
                )
...
def sds_concat(filenames, output=None, include=None):
    ...
    if output is None:
        raise ValueError(
            f"The output kwarg must be specified and be a valid filename to create."
        )
...
def decompress_dataset_internal(
    filename,
    mode=CompressionMode.DecompressFile,
    sharename=None,
    info=False,
    include=None,
    stack=None,
    ...
    if include is not None:
        ...
        if not isinstance(include, list):
            raise ValueError(
                f"The include kwarg must be a list of column names to include. {include}"
            )
    ...
    if sharename is None:
        try:
            ...
            if SDSVerbose:
                verbose_start = VerbosePrint(
                    f"calling rc.MultiDecompressFiles first: {filename[0]} last: {filename[-1]}"
                )
            if stack is not False and info is False and len(filename) == 1:
                ...
                result = rc.MultiPossiblyStackFiles(
                    filename,
                    include=include,
                    folders=folders,
                    filter=filter,
                ...
                if result is None:
                    raise ValueError(
                        f"No data was found in the file {filename} with the specified parameters."
                    )
                ...
                result = rc.MultiDecompressFiles(
                    filename,
                    mode,
                    include=include,
                    folders=folders,
            ...
            if SDSVerbose:
                VerbosePrintElapsed(f"finished rc.MultiDecompressFiles", verbose_start)
        except Exception as e:
            if threads is not None:
                rc.SetThreadWakeUp(savethreads)
            raise e
        ...
        try:
            if SDSVerbose:
                verbose_start = VerbosePrint(
                    f"calling rc.DecompressFiles filename: {filename} sharename: {sharename}"
                )
            ...
            if SDSVerbose:
                VerbosePrintElapsed(
                    f"calling rc.MultiDecompressFiles filename: {filename} sharename: {sharename}",
                    verbose_start,
                )
        except Exception as e:
            if threads is not None:
...
def compress_dataset_internal(
    ...
    if sharename is None:
        if SDSVerbose:
            print(f"calling rc.CompressFile {filename}")
        rc.CompressFile(
            filename,
            metadata,
            listarrays,
        ...
        if SDSVerbose:
            print(f"finished rc.CompressFile")
    else:
        if isinstance(sharename, str):
            sharename = sharename.encode()
...
# ------------------------------------------------------------------------------------
def _rebuild_rootfile(
    path, sharename=None, compress=True, bandsize=None, complevel=None
):
    ...
    meta, arrays, sdsflags, fileheader = decompress_dataset_internal(
        rootpath, sharename=sharename
    )[0]
...
# ------------------------------------------------------------------------------------
def skeleton_from_meta_data(
    container_type, filepath, meta, arrays, meta_tups, file_header
):
...
# ------------------------------------------------------------------------------------
def _init_root_container(
    path, name, sharename=None, info=False, include=None, threads=None
):
    fullpath = path + os.path.sep + name + SDS_EXTENSION
    firstsds = decompress_dataset_internal(
        fullpath, sharename=sharename, info=info, include=include, threads=threads
    )
    ...
    if info:
        ...
        root_struct = skeleton_from_meta_data(
            container_type, path, meta, arrays, meta_tups, fileheader_dict
        )
    else:
        root_struct = container_type._load_from_sds_meta_data(
...
def _include_extra_sds_files(schema, final_sort, include_all_sds=False, include=None):
    ...
    if len(extra_items) > 0:
        ...
        if include_all_sds:
            ...
            warnings.warn(
                f"Found extra .sds information for items {extra_items}.  They will not be included"
            )
    ...
    for item in root_tups:
        ...
        if itemenum & SDSFlag.OriginalContainer and itemenum & SDSFlag.Nested:
            ...
            try:
                ...
                firstsds = decompress_dataset_internal(
                    filename, sharename=sharename, info=True, threads=threads
                )
                ...
                if fileheader["FileType"] == SDSFileType.Struct:
                    ...
                    schema[itemname], dirlist = _build_schema_shared(
                        meta_tups, sharename, prefix, dirlist, threads
                    )
...
def _load_sds_mem(
    ...
    if sys.platform == "linux":
        ...
        root_struct, meta, meta_tups = _init_root_container(
            path,
            "_root",
            sharename=sharename,
            info=info,
...
def _load_sds(
    ...
    if sharename is None:
        ...
        # check directory
            ...
            if has_ext:
                ...
                if sds_isdir(loadpath):
                    ...
                    raise ValueError(
                        f"Failed to load. {path} was not a file or directory and {loadpath} was not a directory."
                    )
            # if no extension, add and check file again
                ...
                if not sds_isfile(loadpath):
                    raise ValueError(
                        f"Failed to load. {path} was not a file or directory and {loadpath} was not a file."
                    )
        if not checkdir:
            return _read_sds(
                loadpath,
        ...
        if sys.platform == "linux":
            ...
            dir, schema, checkdir, root_struct, meta, meta_tups = _load_sds_mem(
                path,
                name=name,
                sharename=sharename,
                info=info,
    ...
    if root_file_found:
        ...
        if final_sort is not None:
            ...
            include_extra = _include_extra_sds_files(
                schema, final_sort, include_all_sds, include=include
            )
        ...
        for k, v in schema.items():
            ...
            try:
                ...
                if item is None:
                    ...
                    root_struct[k] = _sds_load_from_schema(
                        schema,
                        path,
                        dir,
                    ...
                    warnings.warn(
                        f"Found .sds file for item {k}, but was already in struct as {root_struct[k]}. Skipping .sds load.",
                        stacklevel=2,
                    )
            except:
    ...
    if root_file_found and final_sort is not None and include is None:
        ...
        for item in final_sort:
            ...
            if rm_item:
                ...
                if warn_missing:
                    warnings.warn(
                        f"Could not load data for item {item}, file for this item may be missing."
                    )
                missing.append(item)
        for item in missing:
        ...
        if multiload is None:
            ...
            if multiload_idx is None:
                ...
                data = _read_sds(
                    ...
                    include=include,
                    info=info,
                    multiload=multiload[multiload_idx[0]],
                )
                multiload_idx[0] += 1
...
def _build_schema(path, dir, share_prefix="", nodirs=False):
    ...
    if nodirs:
        ...
        files = [
            f[plen:-4]
            for f in dir
            if sds_endswith(f, add=False) and not sds_isdir(os.path.join(path, f))
        ]
...
def save_struct(
    ...
    if has_nested_containers and not onefile:
        ...
        if _sds_path_multi(path, share=sharename, overwrite=overwrite):
            ...
            _sds_from_tree(
                data, path, name=name, sharename=sharename, compress=compress
            )
        else:
            return
    ...
    # otherwise, save as a single .sds file, possibly overwrite
        ...
        path, name, status = _sds_path_single(
            path, share=sharename, overwrite=overwrite
        )
        if status is False:
            return
        ...
        if has_nested_containers and onefile:
            ...
            meta_tups = [
                (name.encode(), arrayflag)
                for name, arrayflag in zip(flatstruct.keys(), arrayflags)
            ]
...
def _read_sds(
    ...
    if ftype == SDSFileType.Array:
        ...
        if info:
            raise NotImplementedError(
                f"SDS info struct not yet supported for onefile saves."
            )
        return TypeRegister.Struct._from_sds_onefile(
            arrs, meta_tups, meta=meta, folders=folders
        )
        ...
        if info:
            ...
            return skeleton_from_meta_data(
                container_type, path, meta, arrs, meta_tups, fileheader_dict
            )
        else:
            ...
            if sections:
                ...
                if len(set_type) == 1 and len(len_type) == 1:
                    ...
                    if only_type == TypeRegister.Dataset:
                        ...
                        if not fail:
                            return TypeRegister.Dataset(arr_dict)
...
def _sds_from_tree(data, path, name=None, compress=True, sharename=None):
    ...
    for k, v in data.items():
        ...
        if hasattr(v, "items") and hasattr(v, "_build_sds_meta_data"):
            ...
            _sds_from_tree(
                v, path, name=new_name, compress=compress, sharename=sharename
            )
            # needs_sds = True
