...
class GroupBy(GroupByOps):
    ...
    def __init__(
        ...
        **kwargs,
            ...
            if isinstance(keys, str):
                ...
                if isinstance(keys, np.ndarray):
                    if len(keys) != dataset.shape[0]:
                        raise TypeError(
                            f'The argument "keys" passed in GroupBy must be the same length as the dataset {len(keys)} vs {dataset.shape[0]}.'
                        )
                    else:
                        name = "col_0"
                        try:
                    ...
                    raise TypeError(
                        f'The argument "keys" passed in GroupBy init must be a list or str.  It must contain column names.'
                    )
        ...
        if ordered is not None and sort_display is not None:
            raise TypeError(
                f"The kwarg ordered and sort_display cannot be used in combination"
            )
    ...
    def gb_keychain(self):
        if self._gb_keychain is None:
            ...
            self._gb_keychain = GroupByKeys(
                self.grouping.uniquedict,
                None,
                sort_display=self._sort_display,
                prebinned=True,
            )
    ...
    def as_categorical(self):
        ...
        return TypeRegister.Categorical(
            idx, _from_categorical=gbkeys, sort_gb=self._sort_display
        )
    ...
    def fill_forward(self, limit=0, fill_val=None, **kwargs):
        ...
        return self.apply_nonreduce(
            fill_forward, fill_val=fill_val, limit=limit, inplace=True
        )
    ...
    def fill_backward(self, limit=0, fill_val=None, **kwargs):
        ...
        """
        return self.apply_nonreduce(
            fill_backward, fill_val=fill_val, limit=limit, inplace=True
        )
