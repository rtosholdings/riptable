...
from ..rt_enum import NumpyCharTypes
from ..rt_enum import TypeRegister as treg
...
class TestDataMeta(type):
    ...
    def dataset_numeric(cls):
        ...
        if cls._dataset_numeric is None:
            cls._dataset_numeric = treg.Dataset(
                {
                    "int8": np.random.randint(low=-128, high=127, size=(cls.length), dtype=np.int8),
                    "uint8": np.random.randint(low=0, high=255, size=(cls.length), dtype=np.uint8),
                    "int16": np.random.randint(low=-32768, high=32767, size=(cls.length), dtype=np.int16),
                    "uint16": np.random.randint(low=0, high=65535, size=(cls.length), dtype=np.uint16),
                    ...
                    "uint32": np.random.randint(low=0, high=4294967295, size=(cls.length), dtype=np.uint32),
    ...
    def dataset_strings(cls):
        # possibly reset dataset with new length
        if cls._dataset_strings is not None and cls._dataset_strings._nrows != cls.length:
    ...
    def dataset_for_gb(cls):
        ...
        if cls._dataset_for_gb is None:
            cls._dataset_for_gb = treg.Dataset({"col_" + str(i): np.random.rand(cls.length) for i in range(10)})
            ...
            cls._dataset_rt_types = treg.Dataset(
                {
                    ...
                    "TSPAN": treg.TimeSpan(
                        np.random.randint(0, 1_000_000_000 * 60 * 60 * 24, cls.length, dtype=np.int64)
                    ),
            ...
            cls._struct_nested = treg.Struct(
                {
                    ...
                    "arr1": arange(5),
                    "ds1": treg.Dataset({"col_" + str(i): np.random.rand(5) for i in range(5)}),
                    "arr2": arange(10),
                    "test_string": "this is my string",
                    "array3": arange(30).astype(np.int8),
                    "nested_struct1": treg.Struct(
                        {
                            "set1": treg.Dataset({"col_" + str(i): np.random.randint(5) for i in range(3)}),
                            "set2": treg.Dataset(
                                {"col_" + str(i): np.random.choice(["a", "b", "c"], 10) for i in range(2)}
                            ),
                            "nested_2": treg.Struct(
                                {
                                    "c1": treg.Categorical(arange(5)),
                                    "c2": treg.Categorical(["aaa", "bbbb", "cccc"]),
                                    "leaf_dataset": treg.Dataset({"col_" + str(i): arange(5) for i in range(3)}),
                                    "array4": arange(20).astype(np.uint64),
                                }
                            ),
                            "set3": treg.Dataset(
                                {"col_" + str(i): np.random.choice([True, False], 10) for i in range(4)}
                            ),
                        }
                    ),
                    "ds2": treg.Dataset({"heading_" + str(i): np.random.rand(5) for i in range(3)}),
                    "int1": 5,
                    "float1": 7.0,
    ...
    def _gen_categorical_data(cls):
        ...
        if cls._int_fa is None:
            ...
            cls._bytes_fa = treg.FastArray(
                np.random.choice(["adam", "brian", "charlie", "david", "edward"], cls.length)
            )
            cls._unicode_fa = treg.FastArray(
                np.random.choice(["amanda", "beth", "catherine", "danielle", "emma"], cls.length),
                unicode=True,
            )
    ...
    def rand_bool(cls):
        ...
        if cls._rand_bool is None:
            cls._rand_bool = np.random.randint(0, 2, cls.length, dtype=bool).view(treg.FastArray)
    ...
    def test_dataset_reduce(
        ...
        result.speedup = treg.FastArray(["{0:.2f}".format(i) + "x" for i in result.speedup])
