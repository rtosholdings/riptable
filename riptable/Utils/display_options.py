...
class DisplayOptions(object):
    ...
    CONSOLE_X = (
        150  # default console width (also calculated by terminalsize.py) TODO: remove
    )
    CONSOLE_Y = (
        25  # default console height (also calculated by terminalsize.py) TODO: remove
    )
    ...
    E_THRESHOLD = (
        6  # power of 10 at which the float flips to scientific notation 10**+/-
    )
    ...
    NUMBER_SEPARATOR_CHAR = (
        DisplayNumberSeparator.Comma
    )  # character for separating , . or _
    ...
    def __new__(cls):
        if cls._CONFIG_LOADED is False:
            ...
            if was_loaded is False:
                print(
                    "No display options found. Creating new display option config file..."
                )
                try:
                    # may fail in windows
    ...
    def save_config(
        ...
        if os.path.exists(full_path) is True:
            if DisplayOptions._AUTO_SAVE is False:
                ...
                if force_overwrite is False:
                    overwrite_prompt = (
                        "File exists at " + full_path + ". Overwrite? (y/n) "
                    )
        ...
        for var in dir(DisplayOptions):
            ...
            if not (callable(getattr(DisplayOptions, var))) and not var.startswith(
                "__"
            ):
