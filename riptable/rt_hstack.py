...
def hstack_any(
    ...
    **kwargs,
        ...
        for container in itemlist:
            for key, item in container.items():
                ...
                if isinstance(item, np.ndarray):
                    ...
                    raise TypeError(
                        f"Unsupported type {type(item)} in container hstack."
                    )
    ...
    def hstack_dict(itemdict, **kwargs):
        ...
        lengths = TypeRegister.FastArray(
            [z.shape[0] for z in itemdict.values()], dtype=np.int64
        ).cumsum()
    ...
    if not isinstance(itemlist, (list, tuple)):
        raise TypeError(
            f"Input must be a list or tuple of datasets. Got {type(itemlist)}"
        )
    ...
    if len(sameclass) != 1:
        raise TypeError(
            f"itemlist must contain objects with the same class: {sameclass!r}"
        )
...
def _hstack_struct(struct_list, destroy: bool = False):
    ...
    for st in struct_list:
        for key, item in st.items():
            ...
            if isinstance(item, TypeRegister.Struct) or TypeRegister.is_array_subclass(
                item
            ):
                dname = type(item).__name__
    ...
    for name, fa_list in alldicts["Array"].items():
        ...
        if len(dims) == 1:
            ...
            if dims == 1:
                ...
                raise ValueError(
                    f"Can only stack arrays of one or two dimensions. Got {dims} dimensions."
                )
        else:
            raise ValueError(
                f"Found conflicting dimensions {dims} for item {name} in multiple structs."
            )
        try:
...
def _hstack_dataset(ds_list: Union[list, tuple], destroy: bool = False):
    ...
    if not isinstance(ds_list, (list, tuple)):
        raise TypeError(
            f"Input must be a list or tuple of datasets. Got {type(ds_list)}"
        )
    ...
    for name, dts in types.items():
        ...
        if len(types[name]) > 1:
            try:
                ...
                safe = all(
                    [
                        t.char in NumpyCharTypes.AllInteger + NumpyCharTypes.AllFloat
                        for t in types[name]
                    ]
                )
                if not safe:
        ...
        if len(ndims[name]) > 1:
            raise ValueError(
                f"Found multiple dimensions for column {name}: {ndims[name]}"
            )
        else:
            dim = ndims[name][0]
            if dim != 1 and dim != 2:
                raise NotImplementedError(
                    f"concat rows only supports columns of 1 or 2 dimensions, got {ndims[name]}"
                )
            else:
                ...
                if dim == 2:
                    ...
                    if len(width) > 1:
                        raise ValueError(
                            f"Found mismatch in axis-1 length for {name}, got {width}"
                        )
                    shapes[name] = width[0]
                ndims[name] = dim
    ...
    for name, t in types.items():
        ...
        for ds in ds_list:
            ...
            if name in ds:
                ...
                inv_count, column_list = _possibly_add_concat_gap(
                    coldim, inv_count, col, column_list, t
                )
        ...
        if inv_count[0] > 0:
            ...
            if len(types) > 1:
                ...
                if common.num <= 13:
                    ...
                    dtype = common
            column_list.append(
                col.fill_invalid(shape=inv_count, inplace=False, dtype=dtype)
            )
...
def _hstack_categorical(cats: list, verbose: bool = False, destroy: bool = False):
    ...
    idx_cutoffs = TypeRegister.FastArray(
        [len(c._fa) for c in cats], dtype=np.int64
    ).cumsum()
    ...
    if mode in (CategoryMode.Dictionary, CategoryMode.IntEnum):
        ...
        unique_cutoffs = [
            TypeRegister.FastArray([len(c) for c in listcodes], dtype=np.int64).cumsum()
        ]
        ...
        unique_cutoffs = [
            TypeRegister.FastArray([len(v) for v in lastv], dtype=np.int64).cumsum()
        ]
...
def _hstack_grouping(
    ...
    if firstgroup.isenum:
        ...
        underlying, newcats = merge_cats(
            underlying,
            listcats,
            unique_cutoffs=cutoffs,
            from_mapping=True,
            ordered=ordered,
        )
        newgroup = TypeRegister.Grouping(
            underlying, categories=dict(zip(newcats[1], newcats[0])), _trusted=True
        )
    else:
        ...
        underlying, newcats = merge_cats(
            listidx, listcats, base_index=base_index, ordered=ordered
        )
        newgroup = TypeRegister.Grouping(
            underlying,
            categories=newcats,
