    ...
    **Row indexing behavior**::
    ...
    def __init__(
        ...
        inputval: Optional[
            Union[
                ArrayCompatible,
                dict,
                Iterable[ArrayCompatible],
                Iterable[Tuple[str, ArrayCompatible]],
                "ItemContainer",
            ]
        ] = None,
        base_index: int = 0,
        ...
        # all other initializers will be flipped to a dictionary, or raise an error
            ...
            inputval = self._init_columns_as_dict(
                inputval, base_index=base_index, sort=sort, unicode=unicode
            )
    ...
    def _init_columns_as_dict(self, columns, base_index=0, sort=True, unicode=False):
        ...
        # record arrays have a void dtype
            ...
            if columns.dtype.char == "V":
                ...
                raise TypeError(
                    f"Can only initialize datasets from arrays that are numpy record arrays."
                )
        ...
        elif isinstance(columns, abc.Iterable) and not isinstance(
            columns, (str, bytes)
        ):
            raise NotImplementedError(
                "Need to implement support for creating a Dataset from an iterable."
            )
                ...
                if isinstance(v, np.ndarray):
                    ...
                    if c == "O":
                        ...
                        if isinstance(
                            ...
                            raise TypeError(
                                f"Cannot convert object array {v} containing {type(v[0])}"
                            )
    ...
    def _possibly_convert(self, name, v, unicode=False):
        ...
        if not isinstance(v, np.ndarray):
            ...
            # pandas Categorical
                ...
                raise TypeError(
                    f"Cannot create a Dataset column out of a {type(v).__name__}."
                )
    ...
    def _check_addtype(self, name, value):
        ...
        if not isinstance(value, np.ndarray):
            if isinstance(value, set):
                raise TypeError(
                    f"Cannot create Dataset column {name!r} out of tuples or sets {value!r}."
                )
            # following pandas
    ...
    def _check_add_dimensions(self, col):
        ...
        if col.ndim > 0:
            ...
            raise ValueError(
                f"Datasets only support columns of 1 or more dimensions. Got {col.ndim} dimensions."
            )
    ...
    def _add_allnames(self, colname, arr, nrows) -> None:
        ...
        if nrows == 0 or nrows == self.get_nrows():
            ...
            raise NotImplementedError(
                f"Cannot set {colname!r} because rows are different lengths."
            )
        ...
        if ncols <= 1:
            ...
            if col_idx in self:
                ...
                if nrows == self.get_nrows() or nrows == 0:
                    ...
                    raise NotImplementedError(
                        f"Cannot set {col_idx!r} because rows are different lengths."
                    )
            ...
            if np.isscalar(value):
                ...
                raise ValueError(
                    "Must have equal len keys and value when setting with a sequence."
                )
        else:
            if np.isscalar(value):
    ...
    def _dataset_compare_check(self, func_name, lhs):
        ...
        if isinstance(lhs, Dataset):
            ...
            if lhs.get_nrows() != nrows:
                ...
                raise ValueError(
                    "The two Datasets have different lengths and cannot be compared"
                )
            else:
    ...
    def imatrix_make(
        ...
        if order != "F" and order != "C":
            raise ValueError(
                f"Invalid order '{order}' specified. The order must be either 'F' or 'C'."
            )
        if order != "F" and inplace:
            raise ValueError(
                "Only the 'F' order may be specified when `inplace` is True."
            )
        ...
        if not isinstance(colnames, list):
            raise TypeError(
                f"Pass in a list of column names such as imatrix_make(['Exch1','Exch2', 'Exch3'])"
            )
    ...
    def imatrix_y(
        self,
        func: Union[callable, str, List[Union[callable, str]]],
        ...
        try:
            ...
            raise ValueError(
                f"No imatrix or failed to create one.  Use imatrix_make to create one."
            )
    ...
    def imatrix_xy(
        ...
        """
        resultY, name, func = self._imatrix_y_internal(
            func, name=name, showfilter=showfilter
        )
    ...
    def fillna(
        ...
        if method is not None:
            if method in ["backfill", "bfill"]:
                return self.apply_cols(
                    FastArray.fill_backward, value, inplace=inplace, limit=limit
                )
            if method in ["pad", "ffill"]:
                return self.apply_cols(
                    FastArray.fill_forward, value, inplace=inplace, limit=limit
                )
            raise KeyError(
                f"fillna: The method {method!r} must be 'backfill', 'bfill', 'pad', 'ffill'"
            )

        if value is None:
            raise ValueError(
                f"fillna: Must specify either a 'value' that is not None or a 'method' that is not None."
            )
    ...
    def apply_cols(
        ...
        **kwargs,
        ...
        _is_ok_list = (
            lambda _x: isinstance(_x, list)
            and len(_x) == nrows
            and all(_is_numeric(_e) for _e in _x)
        )
        _is_ok_array = lambda _x: isinstance(_x, np.ndarray) and _x.shape == (nrows,)
        _is_ok_col_vector = lambda _x: isinstance(_x, np.ndarray) and _x.shape == (
            nrows,
            1,
        )
        _is_for_column = (
            lambda _x: _is_numeric(_x)
            or _is_ok_list(_x)
            or _is_ok_array(_x)
            or _is_ok_col_vector(_x)
        )
        ...
        if not unary:
            ...
            elif isinstance(lhs, Dataset) and all(
                _is_ok_col_vector(_v) for _k, _v in lhs.items() if _k in self
            ):
                return self._operate_iter_input_cols(
                    args, fill_value, func_or_method_name, kwargs, lhs
                )
            elif isinstance(lhs, Struct) and all(
                _is_for_column(_v) for _k, _v in lhs.items() if _k in self
            ):
                return self._operate_iter_input_cols(
                    args, fill_value, func_or_method_name, kwargs, lhs
                )
        ...
        for colname, array in self.items():
            ...
            if colname in self.label_get_names() and not labels:
                ...
                if isinstance(fill_value, dict):
                    ...
                    fval = fill_value
                od[colname] = _operate_on_array(
                    array, func_or_method_name, *args, **kwargs
                )
        ...
        if self.footer_get_values():
            try:
                num_labels = (
                    len(self.label_get_names()) if self.label_get_names() else 0
                )
                arrays = []
                for self_footervals in self.footer_get_values().values():
                    array = FastArray(self_footervals[num_labels:])
                    arrays += [
                        _operate_on_array(array, func_or_method_name, *args, **kwargs)
                    ]
                footers = self._construct_new_footers(
                    arrays, num_labels, summary_colnames
                )
            except:
                footers = None
    ...
    def _operate_iter_input_cols(
        self, args, fill_value, func_or_method_name, kwargs, lhs
    ):
        """
        ...
        for colname in self.keys():
            ...
            if (
                conform
                and self.summary_get_names()
                and colname in self.summary_get_names()
            ):
            ...
            if lhs_colname in lhs and colname not in self.label_get_names():
                ...
                self1 = self1.apply_cols(
                    func_or_method_name, *args1, fill_value=fill_value, **kwargs
                )
        ...
        ret_obj = self._add_labels_footers_summaries(
            type(self)(od), summary_colnames, footers
        )
        return ret_obj
        ...
        def _columns_conform(func, left_or_right="left"):
            ...
            if self_names:
                ...
                if left_or_right == "right":
                    if (
                        self_indexes
                        != list(range(len(self.keys())))[-len(self_names) :]
                    ):
                        return False
            return True
    ...
    def asrows(
        self,
        as_type: Union[str, type] = "Dataset",
        dtype: Optional[Union[str, np.dtype]] = None,
    ):
        """
    ...
    def to_pandas(
        self, unicode: bool = True, use_nullable: bool = True
    ) -> "pd.DataFrame":
        """
        ...
        for key, col in self.items():
            ...
            if isinstance(col, TypeRegister.Categorical):
                ...
                elif col.category_mode in (
                    CategoryMode.Dictionary,
                    CategoryMode.MultiKey,
                ...
                categories = (
                    _to_unicode_if_string(col.category_array)
                    if unicode
                    else col.category_array
                )
                data[key]: pd.Categorical = pd.Categorical.from_codes(
    ...
    def as_pandas_df(self):
        ...
        """
        warnings.warn(
            "as_pandas_df is deprecated and will be removed in future release, "
    ...
    def from_pandas(cls, df: "pd.DataFrame", tz: str = "UTC") -> "Dataset":
        ...
        for key in df.columns:
            ...
            if iscat or isinstance(col, pd.Categorical):
                ...
                data[key] = np.asarray(
                    col.fillna(INVALID_DICT[dtype.numpy_dtype.num]),
                ...
                try:
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Unable to convert a datetime array with timezone={}".format(
                                ptz
                            )
                        )
                except AttributeError:
                    _tz = tz
                data[key] = TypeRegister.DateTimeNano(
                    np.asarray(col, dtype="i8"), from_tz="UTC", to_tz=_tz
                )
            elif dtype_kind == "m":
    ...
    # -------------------------------------------------------------
    def drop_duplicates(
        self, subset=None, keep: Union[bool, str] = "first", inplace: bool = False
    ) -> "Dataset":
        """
    ...
    def sorts_on(self) -> None:
        ...
        if self._col_sortlist is None:
            warnings.warn(
                f"sort_view was not called first.  Display sorting will remain off."
            )
            return
    ...
    def merge_lookup(
        ...
        if columns_left:
            raise ValueError(
                "'columns_left' cannot be specified when performing an in-place merge_lookup."
            )
        ...
        left_on = rt_merge._extract_on_columns(
            on, left_on, True, "on", is_optional=False
        )
        right_on = rt_merge._extract_on_columns(
            on, right_on, False, "on", is_optional=False
        )
    ...
    def _prepare_display_data(self):
        ...
        mainkeys = [
            c
            for c in self
            if c not in leftkeys and c not in rightkeys and c not in sortkeys
        ]
        ...
        if footers is not None:
            ...
            footerrows = [
                padding + [rowname] + footervals
                for rowname, footervals in footerdict.items()
            ]
            # column footer tuples with string repr of each value
            footer_tups = [
                [ColHeader(format_scalar(fval), 1, 0) for fval in frow]
                for frow in footerrows
            ]
    ...
    def transpose(
        ...
        t_array, colnames = self.imatrix_make(
            colnames=colnames, cats=cats, gb=gb, inplace=False, retnames=True
        )
        ...
        **kwargs,
        ...
        if axis == 1:
            ...
            if not any(noncomp):
                ...
                return FastArray(
                    [func(_r, **kwargs) for _r in self.asrows(as_type="array")]
                )
            ...
            def _row(_i):
                ...
                if _keep.all():
                    return _r
                return [
                    _x for _i, _x in enumerate(_r) if _keep[_i]
                ]  # cannot use np.take!!!

            # TJD this code is slow and needs review
            return np.array(
                [func(_row(_i), **kwargs) for _i in range(self.get_nrows())]
            )
        ...
        if axis is None:
            if not any(noncomp):
                ...
                return func(
                    np.reshape([self.col_get_value(_k) for _k in self.keys()], -1),
                    **kwargs,
                )
            # respects noncomputable cols.
            # 290 µs ± 1.86 µs per loop (mean ± std. dev. of 7 runs, 1000 loops each)
            bycols = self.reduce(
                func, axis=0, as_dataset=True, fill_value=fill_value, **kwargs
            )
    ...
    def argmax(self, axis=0, as_dataset=True, fill_value=None):
        return self.reduce(
            argmax, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )

    def argmin(self, axis=0, as_dataset=True, fill_value=None):
        return self.reduce(
            argmin, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )

    def normalize_zscore(self, axis=0, as_dataset=True, fill_value=None):
        return self.reduce(
            normalize_zscore, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )

    def normalize_minmax(self, axis=0, as_dataset=True, fill_value=None):
        return self.reduce(
            normalize_minmax, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )
    ...
    def mean(self, axis=0, as_dataset=True, fill_value=None):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            mean, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )

    def var(self, axis=0, ddof=1, as_dataset=True, fill_value=None):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            var, axis=axis, as_dataset=as_dataset, fill_value=fill_value, ddof=ddof
        )

    def std(self, axis=0, ddof=1, as_dataset=True, fill_value=None):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            std, axis=axis, as_dataset=as_dataset, fill_value=fill_value, ddof=ddof
        )

    def median(self, axis=0, as_dataset=True, fill_value=None):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            median, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )
    ...
    def nanargmax(self, axis=0, as_dataset=True, fill_value=None):
        return self.reduce(
            nanargmax, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )

    def nanargmin(self, axis=0, as_dataset=True, fill_value=None):
        return self.reduce(
            nanargmin, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )

    def nansum(self, axis=0, as_dataset=True, fill_value=None):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            nansum, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )

    def nanmean(self, axis=0, as_dataset=True, fill_value=None):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            nanmean, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )

    def nanvar(self, axis=0, ddof=1, as_dataset=True, fill_value=None):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            nanvar, axis=axis, as_dataset=as_dataset, fill_value=fill_value, ddof=ddof
        )

    def nanstd(self, axis=0, ddof=1, as_dataset=True, fill_value=None):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            nanstd, axis=axis, as_dataset=as_dataset, fill_value=fill_value, ddof=ddof
        )

    def nanmedian(self, axis=0, as_dataset=True, fill_value=None):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            nanmedian, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )

    def nanmin(self, axis=0, as_dataset=True, fill_value=min):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            nanmin, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )

    def nanmax(self, axis=0, as_dataset=True, fill_value=max):
        """See documentation of :meth:`~rt.rt_dataset.Dataset.reduce`"""
        return self.reduce(
            nanmax, axis=axis, as_dataset=as_dataset, fill_value=fill_value
        )
    ...
    @classmethod
    def concat_rows(
        cls, ds_list: Iterable["Dataset"], destroy: bool = False
    ) -> "Dataset":
        """
        Stacks columns from multiple datasets.
    ...
    def concat_columns(
        ...
        if on_mismatch != "ignore":
            if len(dups) > 0:
                if on_mismatch == "warn":
                    warnings.warn(
                        f"concat_columns() duplicate column mismatch: {dups!r}"
                    )
    ...
    def as_matrix(self, save_metadata=True, column_data={}):
        ...
        for col in range(ncols):
            ...
            (
                ...
                is_categorical,
                category_values,
            ) = self._normalize_column(self[columns[col]], field_key)
    ...
    def apply(self, funcs, *args, check_op: bool = True, **kwargs):
        ...
        * Not perform in-place operations on the input array. Arrays
        ...
        for f in funcs:
            if not callable(f):
                raise TypeError(
                    f"{f} is not callable. Could not be applied to dataset."
                )
    ...
    def from_tagged_rows(cls, rows_iter):
        ...
        for row in rows_iter:
            if isinstance(row, tuple) and hasattr(
                row, "_fields"
            ):  # proxy for a namedtuple
                ...
                raise TypeError(
                    f"{cls.__name__}.from_tagged_rows: input must be iterable of dict or Struct."
                )
        ...
        if len(set(keys.values())) != 1:
            raise NotImplementedError(
                f"{cls.__name__}.from_tagged_rows(): All rows must have same keys."
            )
        retval = {
            _k: [] for _k in sorted(keys)
        }  # no reason to priv. the key order of any one row
        if n_have_getitem == 0:
            for row in rows:
                for _k in row:
            ...
            for row in rows:
                for _k in row:
                    retval[_k].append(
                        row[_k] if hasattr(row, "__getitem__") else getattr(row, _k)
                    )
        return cls(retval)
    ...
    def from_rows(cls, rows_iter, column_names):
        ...
        for row in rows_iter:
            if isinstance(row, (dict, Struct, Dataset)):  # other dict types?
                raise TypeError(
                    f'{cls.__name__}.from_rows: rows can not be "dictionaries".'
                )
            if len(row) != ncols:
                raise ValueError(
                    f"{cls.__name__}.from_rows: all rows must have same length as column_names."
                )
    ...
    def from_jagged_dict(cls, dct, fill_value=None, stacked=False):
        ...
        for k, v in dct.items():
            if num_in_rows == 0:
                ...
                if len(v) != num_in_rows:
                    raise ValueError(
                        f"{cls.__name__}.from_jagged_ rows: all values must "
                        + "have same length."
                    )
        ...
        if not stacked:
            ds = cls()
            for k, v in dct.items():
                ids = Dataset.from_jagged_rows(
                    v, column_name_base=k, fill_value=fill_value
                )
                for ik in ids.keys():
                    ds[ik] = ids[ik]
            return ds
    ...
    def pivot(
        ...
        if not isinstance(labels, list) or len(labels) == 0:
            raise ValueError(
                'The parameter "labels" must exist and be passed as a string or list of strings.'
            )

        if columns is None or not isinstance(columns, (str, list)):
            raise ValueError(
                'The parameter "columns" must exist and be passed as a string or list of strings.'
            )
        if np.isscalar(columns):
            columns = [columns]
        if not isinstance(columns, list) or len(columns) == 0:
            raise ValueError(
                'The parameter "columns" must exist and be passed as a list of one or more strings.'
            )
    ...
    def equals(
        ...
        if exact:
            ...
            try:
                result = self.apply_cols(isnan, labels=labels) & other.apply_cols(
                    isnan, labels=labels
                )
                result |= self == other
            except:
