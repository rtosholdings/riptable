    ...
    def clear(cls, sharename=None):
        ...
        if sys.platform == "linux":
            ...
            raise NotImplementedError(
                f"This feature is for Linux only. To clear shared memory in Windows, exit  your python session."
            )
    ...
    def check_shared_memory_limit(cls):
        if sys.platform == "linux":
            ...
            if size > cls.SM_LIMIT:
                ...
                warnings.warn(
                    f"!!!Shared memory is using {readable}. Consider using sm.clear() to remove."
                )
