...
class Categorical_Test(unittest.TestCase):
    def _notimpl(self):
        self.skipTest("This test needs to be implemented.")
    ...
    def test_ctor_list(self):
        ...
        self.assertEqual(
            c_bytes.dtype,
            np.int8,
            msg=f"Dtype {c_bytes.dtype} was not correct for construction from small list.",
        )
        self.assertEqual(
            len(c_bytes),
            5,
            msg=f"Length of underlying index array was incorrect for construction from bytes.",
        )
        unique_bytes = np.unique(list_bytes)
        self.assertTrue(
            np.all(c_bytes._categories_wrap._list == unique_bytes),
            msg=f"Categories did not generate a unique list of categories from input bytes list.",
        )
        ...
        self.assertEqual(
            c_unicode.dtype,
            np.int8,
            msg=f"Dtype {c_unicode.dtype} was not correct for construction from small list.",
        )
        self.assertEqual(
            len(c_unicode),
            5,
            msg=f"Length of underlying index array was incorrect for construction from unicode.",
        )
        self.assertEqual(
            len(c_unicode._categories_wrap),
            4,
            msg=f"Length of unique categories was incorrect for construction from unicode.",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            c_true_unicode.dtype,
            np.int8,
            msg=f"Dtype {c_true_unicode.dtype} was not correct for construction from small list.",
        )
        self.assertEqual(
            len(c_true_unicode),
            5,
            msg=f"Length of underlying index array was incorrect for construction from true unicode.",
        )
        self.assertEqual(
            len(c_true_unicode._categories_wrap),
            4,
            msg=f"Length of unique categories was incorrect for construction from true unicode.",
        )
        self.assertEqual(
    ...
    def test_ctor_nparray(self):
        ...
        self.assertEqual(
            len(c_unicode._categories_wrap._list),
            4,
            msg=f"Length of unique categories was incorrect for construction from unicode.",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(c_true_unicode._categories_wrap._list),
            4,
            msg=f"Length of unique categories was incorrect for construction from true unicode.",
        )
        self.assertEqual(
        ...
        for v in valid_v:
            ...
            for c in valid_c:
                ...
                with self.assertRaises(
                    ValueError,
                    msg=f"Failed to warn when not all array values were in provided categories. values: {v} cats: {c}",
                ):
                    cat = Categorical(v, c)
    ...
    def test_compare_enum_int(self):
        ...
        for d in (LikertDecision, decision_dict):
            ...
            for name, correct in zip(compare_func_names, valid_idx_correct):
                ...
                self.assertTrue(
                    was_correct,
                    msg=f"Categorical enum comparison failed with good integer index on {name} operation. {c.view(FastArray)} code: {valid_idx}",
                )
            ...
            for name, correct in zip(compare_func_names, bad_idx_correct):
                ...
                self.assertTrue(
                    was_correct,
                    msg=f"Categorical enum comparison failed with good integer index on {name} operation",
                )
    ...
    def test_compare_enum_str(self):
        ...
        for d in (LikertDecision, decision_dict):
            ...
            for name, correct in zip(compare_func_names, valid_idx_correct):
                ...
                self.assertTrue(
                    was_correct,
                    msg=f"Categorical enum comparison failed with good category string on {name} operation",
                )
            ...
            for name in compare_func_names:
                ...
                with self.assertRaises(
                    ValueError,
                    msg=f"Failed to raise an error when performing comparison operation {name} on categorical from enum with invalid category",
                ):
                    result = func(bad_idx)
    ...
    def test_getitem_enum_int(self):
        ...
        for idx in range(5):
            self.assertEqual(
                correct_strings[idx],
                c[idx],
                msg=f"Failed to return correct string for valid index in categorical.",
            )

        # getitem bad init
        with self.assertRaises(
            IndexError,
            msg=f"Failed to pass bad integer index to underlying fastarray in categorical enum getitem",
        ):
            result = c[5]
    ...
    def test_ctor_pandas_cat(self):
        ...
        self.assertTrue(
            cats_match,
            msg=f"Failed to create matching categories from pandas categorical",
        )
    ...
    def test_ctor_list_unique(self):
        ...
        self.assertTrue(
            cats_match,
            msg=f"Failed to create matching categories from unique category input.",
        )
    ...
    def test_ctor_garbage(self):
        with self.assertRaises(
            TypeError,
            msg=f"Failed to raise error with bad categorical initialization 1,2",
        ):
            c = Categorical(1, 2)
    ...
    def test_init_with_dtype(self):
        ...
        for dt in int_types:
            ...
            self.assertEqual(
                c.dtype,
                dt,
                msg=f"Failed to force the correct dtype {dt} for categorical.",
            )

        for dt in float_types + uint_types:
            with self.assertRaises(
                TypeError,
                msg=f"Failed to raise error when forcing non integer dtype {dt} for categorical.",
            ):
                c = Categorical(arr, dtype=dt)
    ...
    def test_categories_len(self):
        ...
        self.assertEqual(len(cats_from_enum), 144)
    ...
    def test_get_categories(self):
        ...
        get_cats_match = bool(
            np.all(cats_from_list.get_categories() == cats_from_enum.get_categories())
        )
        self.assertTrue(get_cats_match)
    ...
    def test_is_in_unique_strings(self):
        ...
        # valid bytes
        c = Categorical(
            values, good_cats, ordered=True, base_index=1, unicode=False, filter=None
        )
        ...
        # valid unicode
        c = Categorical(
            values, good_cats, ordered=True, base_index=1, unicode=True, filter=None
        )
        cats = c._categories_wrap
        self.assertEqual(len(c), 7)
        self.assertEqual(max(c._fa), 4)
    ...
    def test_match_str_to_category(self):
        ...
        self.assertTrue(isinstance(matching_char, bytes))
        with self.assertRaises(TypeError):
    ...
    def test_align(self):
        ...
        self.assertTrue((cm[0].categories() == cm[2].categories()).all())
        c1 = Categorical([1, 1, 3, 2, 2], [1, 2, 3, 4, 5], from_matlab=True)
        c2 = Categorical([2, 2, 4, 4, 3], [1, 2, 3, 4, 5], from_matlab=True)
        cm = Categorical.align([c1, c2])
        self.assertTrue((cm[0] == [1, 1, 3, 2, 2]).all())
        self.assertTrue((cm[1] == [2, 2, 4, 4, 3]).all())
    ...
    def test_getitem_empty(self):
        ...
        with self.assertRaises(IndexError):
            empty_np = c[np.array([])]
            self.assertTrue(isinstance(empty_np, Categorical))
    ...
    def test_iter_groups(self):
        ...
        for i, tup in enumerate(c.iter_groups()):
            ...
            self.assertTrue(bool(np.all(tup[1] == correct_idx[i])))
    ...
    @pytest.mark.xfail(
        resaon="After invalid_set, the custom invalid value is not displayed."
    )
    def test_invalid_set(self):
        ...
        self.assertEqual(c[3], custom_invalid)
        self.assertTrue(c.isnan()[3])
        self.assertEqual(len(c.category_array), 4)
    ...
    def test_getitem_slice(self):
        ...
        self.assertTrue(match_fa)
        self.assertEqual(len(result), 3)
        self.assertEqual(len(result._categories_wrap), 3)
    ...
    def test_fill_invalid(self):
        ...
        assert_array_equal(
            expected, c.category_dict[next(iter(c.category_dict))]
        )  # values of first key
    ...
    def test_categories_shallow_copy(self):
        ...
        self.assertTrue(
            ...
            msg=f"Categories did not use same str_to_int dictionary after shallow copy.",
        )
        self.assertTrue(
            original_cats._int_to_str_dict is new_cats._int_to_str_dict,
            msg=f"Categories did not use same int_to_str dictionary after shallow copy.",
        )
    ...
    def test_set_item_str_index(self):
        ...
        self.assertTrue(
            is_correct,
            msg=f"Category was not correctly changed with set item on a string.",
        )
        with self.assertRaises(
            ValueError,
    ...
    def test_set_item_int_index(self):
        ...
        self.assertTrue(
            is_correct,
            msg=f"Category was not correctly changed with set item on an int.",
        )
        with self.assertRaises(
            ValueError,
            msg=f"Failed to stop categories from being added when auto add set to False.",
        ...
        for dt, sz in dtype_sizes.items():
            ...
            if len(c._categories_wrap._list) >= sz:
                self.assertEqual(
                    c.dtype,
                    dt,
                    msg=f"Categorical did not set dtype to {dt} for array of size {sz}.",
                )
    ...
    def test_getitem_int(self):
        ...
        with self.assertRaises(IndexError, msg=f"Failed to assert index out of range."):
            d = c[10]
    ...
    def test_getitem_int_mask(self):
        ...
        for mask in [py_mask, np.array(py_mask)]:
            d = c[mask]
            self.assertTrue(
                isinstance(d, Categorical),
                msg=f"Get item with integer mask did not return a categorical. Returned {type(d).__name__} instead.",
            )
            self.assertEqual(
                len(d),
                len(mask),
                msg=f"Get item with integer mask did not return categorical of {len(mask)}. returned {len(d)} instead.",
            )
            has_same_cats = bool(
                np.all(d._categories_wrap._list == c._categories_wrap._list)
            )
            self.assertTrue(
                has_same_cats,
                msg=f"Failed to copy the same categories to new categorical after getitem with integer mask.",
            )
            d = c[[0, 10]]
            self.assertTrue(
                d._fa[1] == 0, msg=f"Failed to put invalid for out of range index."
            )
    ...
    def test_getitem_bool_mask(self):
        ...
        for mask in [py_mask, np.array(py_mask)]:
            ...
            self.assertEqual(
                5,
                len(d),
                msg=f"Length {len(d)} did not match 5 in categorical getitem with a boolean array of the same size.",
            )
            has_same_cats = bool(
                np.all(d._categories_wrap._list == c._categories_wrap._list)
            )
    ...
    def test_getitem_single_string(self):
        ...
        self.assertFalse(
            has_true,
            msg=f"Failed to return an array of all false for getitem with {idx}",
        )
        self.assertTrue(
            isinstance(d, FastArray),
        ...
        self.assertTrue(
            is_correct, msg=f"Did not return the correct array for getitem with {idx}"
        )
        self.assertTrue(
            isinstance(d, FastArray),
            msg=f"Get item input {idx} did not return FastArray",
    ...
    def test_getitem_numeric_categories(self):
        ...
        self.assertTrue(isinstance(c[0], (int, np.integer)))
        nums = nums.astype(np.float32)
        c = Categorical(nums)
        self.assertEqual(c[0], 1.0)
        self.assertTrue(
            isinstance(c[0], (float, np.floating)),
            msg=f"Expected float, got {type(c[0])}",
        )
    ...
    def test_compare_check(self):
        ...
        for name, correct in compare_ops.items():
            ...
            self.assertTrue(
                is_correct,
                msg=f"Compare operation betweeen two equal categoricals did not return the correct result.",
            )
    ...
    def test_compare_return_type(self):
        ...
        for s in scalars:
            for op in compare_ops:
                ...
                result = func(s)
                self.assertTrue(
                    isinstance(result, FastArray),
                    msg=f"comparison {op} with input {s} did not return FastArray",
                )
                self.assertEqual(
                    result.dtype.char,
                    '?',
    ...
    def test_isnan(self):
        ...
        self.assertTrue(bool(np.all(is_correct == c.isnan())))
        self.assertTrue(bool(np.all(is_not_correct == isnotnan(c))))
        self.assertTrue(bool(np.all(is_not_correct == c.isnotnan())))
    ...
    def test_get_categories(self):
        ...
        self.assertTrue(isinstance(catsdict, dict))
        self.assertEqual(len(catsdict), 1)
        with self.assertRaises(TypeError):
        ...
        self.assertEqual(len(catsdict), 1)
        catscodes = c.category_codes
        self.assertTrue(isinstance(catscodes, np.ndarray))
        catsmapping = c.category_mapping
        self.assertTrue(isinstance(catsmapping, dict))
        ...
        catsdict = c.category_dict
        self.assertTrue(isinstance(catsdict, dict))
        self.assertTrue(len(catsdict), 2)
        with self.assertRaises(TypeError):
            catscodes = c.category_codes
        with self.assertRaises(TypeError):
            catsmapping = c.category_mapping
    ...
    def test_display_properties(self):
        ...
        self.assertEqual(item_format.justification, DisplayJustification.Left)
    ...
    def test_get_string_mode_nums(self):
        ...
        self.assertFalse(c._categories_wrap.isbytes)
    ...
    def test_total_size(self):
        ...
        self.assertEqual(c._total_size, 45)

        c = Categorical([arange(5, dtype=np.int64), arange(5, dtype=np.int64)])
        self.assertEqual(c._total_size, 85)
    ...
    def test_expand_dict(self):
        ...
        dictlist = list(d.values())
        self.assertTrue(bool(np.all(dictlist[0] == arange(1, 4))))
    ...
    def test_stack_multikey(self):
        ...
        self.assertTrue(isinstance(hstack_ds, Dataset))

        # dataset hstack from load
        stack_load_ds = load_sds(paths, stack=True)
        self.assertTrue(isinstance(stack_load_ds, PDataset))

        # multikey cat hstack
        hstack_mkcats = hstack([c1, c2])
        self.assertTrue(isinstance(hstack_mkcats, Categorical))
        ...
        self.assertTrue(isinstance(hstack_cstrs, Categorical))
        hstack_cflts = hstack([c1_flt, c2_flt])
        self.assertTrue(isinstance(hstack_cflts, Categorical))

        self.assertTrue(bool(np.all(hstack_strs == hstack_cstrs.expand_array)))
        self.assertTrue(bool(np.all(hstack_flts == hstack_cflts.expand_array)))

        mktup = [*hstack_mkcats.category_dict.values()]
        self.assertTrue(bool(np.all(hstack_mkcats._expand_array(mktup[0]) == fa_str)))
        self.assertTrue(bool(np.all(hstack_mkcats._expand_array(mktup[1]) == fa_flt)))

        mktup2 = [*stack_load_ds.mkcat.category_dict.values()]
        self.assertTrue(
            bool(np.all(stack_load_ds.mkcat._expand_array(mktup2[0]) == fa_str))
        )
        self.assertTrue(
            bool(np.all(stack_load_ds.mkcat._expand_array(mktup2[1]) == fa_flt))
        )

        mktup3 = [*hstack_ds.mkcat.category_dict.values()]
        self.assertTrue(
            bool(np.all(hstack_ds.mkcat._expand_array(mktup3[0]) == fa_str))
        )
        self.assertTrue(
            bool(np.all(hstack_ds.mkcat._expand_array(mktup3[1]) == fa_flt))
        )
    ...
    def test_as_string_array(self):
        ...
        self.assertTrue(
            isinstance(c.as_string_array, FastArray),
            msg=f"Categorical did not return a fastarray in as_string_array",
        )
        self.assertTrue(
            is_equal,
            msg=f"Categorical returned an incorrect string array {c.as_string_array} view of itself. Expected {f}",
        )
    ...
    def test_category_add(self):
        ...
        assert (
            e in cat._categories
        ), "expect the added category to be added to the Categorical._categories"
        assert (
            e in cat.category_array
        ), "expect the added category to be added to the Categorical.category_array"
        assert (
            e in cat.category_dict
        ), "expect the added category to be added to the Categorical.category_dict"
    ...
    def test_category_remove(self):
        ...
        assert (
            e not in cat
        ), "expect the removed category to be removed from the Categorical"
        assert (
            e not in cat._categories
    ...
    def test_assinglekey(self):
        ...
        c2 = c.as_singlekey(ordered=True)
        self.assertTrue(np.all(c1.expand_array == c2.expand_array))
        self.assertTrue(np.all(c3.expand_array == c2.expand_array))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            [
                Categorical(data)
                for data in get_categorical_data_factory_method(
                    [CategoryMode.StringArray, CategoryMode.NumericArray]
                )
            ],
        ...
        pytest.param(
            [
                Categorical(data)
                for data in get_categorical_data_factory_method(CategoryMode.MultiKey)
            ],
            marks=[
                pytest.mark.xfail(
...
def test_shift_cat(categoricals):
    ...
    for categorical in categoricals:
        ...
        for i in range(
            -cat_len + 1, cat_len
        ):  # exhaustive shift of all Categorical values.
        ...
        for i in list(range(-cat_len - 2, -cat_len)) + list(
            range(cat_len, cat_len + 2)
        ):
            shift_cat = categorical.shift_cat(i)
            assert_array_equal(shift_cat.category_array, categorical.category_array)
...
def test_shrink(categoricals, misc, inplace):
    for categorical in categoricals:
        cat = categorical.copy(
            deep=True
        )  # deep copy so test data remains unchanged with inplace shrinks
        ...
        assert isinstance(
            shrink_cat, Categorical
        ), "shrink_cat should be a Categorical."
        ...
        if misc is None:  # TODO handle the misc not None case
            ...
            for arr, expected_arr in zip(
                shrink_cat.category_dict.values(), cat.category_dict.values()
            ):
...
def test_sds(categoricals, tmpdir):
    ...
    for i, cat in enumerate(categoricals):
        ...
        if (
            not cat.ismultikey
        ):  # MultiKey Categorical's do not support category_array operation
            assert_array_equal(cat2.category_array, cat.category_array)
        for actual, expected in zip(
            cat2.category_dict.values(), cat.category_dict.values()
        ):
        ...
        for actual, expected in zip(
            cat3.category_dict.values(), cat.category_dict.values()
        ):
            assert_array_equal(actual, expected)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            marks=[
                pytest.mark.xfail(
                    reason="NotImplementedError: Add categories not supported for MultiKey Categoricals"
                )
            ],
        ),
    ],
...
def test_auto_add(cats):
    for cat in cats:
        ...
        with pytest.raises(
            ValueError
        ):  # cannot automatically add categories while auto_add_categories is False
            cat[first_index] = beta
...
def test_categorical_convert(values):
    ...
    assert (
        missing_category not in cat.category_dict[next(iter(cat.category_dict))]
    )  # values of first key
    ...
    for e in categories[:-1]:
        ...
        assert (
            e in cat.category_dict[next(iter(cat.category_dict))]
        )  # values of first key


@pytest.mark.xfail(
    reason="RIP-396 - category not in Categorical, but is in Categorical.category_array"
)
...
@pytest.mark.parametrize(
    "values", [list("abcdef"), [b"a", b"b", b"c", b"d", b"e", b"f"]]
)
