...
def run_all(argv, verbosity=3):
    if len(argv) > 1:
        try:
            ...
            raise SystemExit(
                "Usage: %s [ verbosity_int (def=%d) ]" % (argv[0], verbosity)
            )
    pkg_name = os.path.basename(os.path.dirname(__file__))
    failures = []
    for test in get_all_tests():
        print("==> Running tests: %s <==\n" % test, file=sys.stderr)
        module_name = "riptable.{pkg_name}.{mod_name}".format(
            pkg_name=pkg_name, mod_name=test
        )
        module = __import__(module_name, fromlist=[""])
        utest = unittest.main(
            module=module, exit=False, argv=[module_name], verbosity=verbosity
        ).result
