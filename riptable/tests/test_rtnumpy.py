...
class TestRTNumpy:
    ...
    def test_where(self):
        ...
        assert_array_equal(
            result, arr, err_msg=f"Results did not match for where. {arr} vs. {result}"
        )
    ...
    def test_unique(self):
        ...
        assert_array_equal(
            z1, z2, err_msg=f"Results did not match for unique inverse. {z1} vs. {z2}",
        )
...
class TestNanmax:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                [math.nan, math.nan, math.nan, math.nan, math.nan], id="list-float"
            ),
            pytest.param(
                {np.nan},
                id="set-float",
                marks=pytest.mark.skip("Broken in both numpy and riptable."),
            ),
    ...
    def test_unordered_categorical_disallowed(self):
        ...
        cat = rt.Categorical(
            ["PA", "NY", "NY", "AL", "LA", "PA", "CA", "IL", "IL", "FL", "FL", "LA"],
            ordered=False,
        )
    ...
    @pytest.mark.xfail(
        reason="RIP-417: nanmax does not (yet) support returning a category scalar."
    )
...
class TestNanmin:
    ...
    @pytest.mark.xfail(
        reason="RIP-417: nanmin does not (yet) support returning a category scalar."
    )
    def test_ordered_categorical_returns_scalar(self):
        """
