...
class Struct_Test(unittest.TestCase):
    ...
    def test_col_map_02(self):
        st = Struct({_k: list(range(_i * 10, (_i + 1) * 10)) for _i, _k in enumerate("abcdefghijklmnopqrst")})
        cmap = {
            "a": "x",  # 1-transition
            "b": "c",
            "c": "b",  # 2-cycle
    ...
    def test_col_moves(self):
        st = Struct({_k: list(range(_i * 10, (_i + 1) * 10)) for _i, _k in enumerate("abcdefghijklmnop")})
    ...
    def test_as_dictionary(self):
        od = {_k: list(range(_i * 10, (_i + 1) * 10)) for _i, _k in enumerate("abcdefghij")}
        st = Struct(od)
        dt1 = st.asdict()
        dt2 = st.asdict(copy=True)
        self.assertEqual(self._sum_dict(dt1), self._sum_dict(od))
        self.assertIsInstance(dt1, dict)
        self.assertEqual(list(st.asdict(sublist=["a", "c", "d"]).keys()), ["a", "c", "d"])
        with self.assertRaises(AttributeError):
    ...
    def test_concat(self):
        st1 = Struct(
            {
                "ds": TypeRegister.Dataset({"col_" + str(i): np.random.rand(5) for i in range(5)}),
                "arr": arange(5),
    ...
    def test_categorical(self):
        ...
        ds = Dataset({"A": ["a", "b"], "B": ["c", "d"], "C": ["a", "μεαν"], "D": [1, 2]})
    ...
    def _test_output(self):
        ...
        self.assertEqual([hd.col_name for hd in headers[0]], ["Name", "Type", "Rows", "0", "1", "2"])
        self.assertEqual(
...
9   j      slice      0                  """,
    ...
    def test_total_sizes(self):
        ...
        self.assertLess(physical, logical, "The physical size is not less than the logical size.")
    ...
    def test_total_sizes_with_categorical(self):
        ...
        self.assertGreaterEqual(physical, np.asarray(st.c).nbytes + st.c.category_array.nbytes)
