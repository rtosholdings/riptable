...
class PGroupby_Test(unittest.TestCase):
    def test_groupbyhash_cutoffs(self):
        ...
        self.assertTrue(bool(np.all(gb_arr["iFirstKey"] == ifirst_vals[: ifirst_cutoffs[0]])))

        self.assertTrue(bool(np.all(gb_arr2["iKey"] == ikey[15:])))
        self.assertTrue(bool(np.all(gb_arr2["iFirstKey"] == ifirst_vals[ifirst_cutoffs[0] :])))
        self.assertEqual(gb_arr3["unique_count"], (gb_arr["unique_count"] + gb_arr2["unique_count"]))
    ...
    def test_cutoffs_vs_multikey(self):
        ...
        self.assertTrue(bool(np.all(gb_mk["iFirstKey"][: ifirst_cutoffs[0]] == ifirst_vals[: ifirst_cutoffs[0]])))
        self.assertTrue(bool(np.all(gb_mk["iFirstKey"][ifirst_cutoffs[0] :] == ifirst_vals[ifirst_cutoffs[0] :] + 15)))
