...
class AccumTable_Test(unittest.TestCase):
    ...
    def test_accum_cols_multikey(self):
        ...
        accum = rt.accum_cols(data.MultiKeyCat, [data.Count, data.PlusMinus], ["Count", "PlusMinus"])
        ...
        accum_expected.footer_set_values("Total", {"Exch": "Total", "Count": 12.0, "PlusMinus": 0.0})
    ...
    def test_accum_ratiop(self):
        ...
        # Invalid input
        with self.assertRaises(ValueError, msg=f"Failed to raise an error when passing invalid norm_by arg"):
