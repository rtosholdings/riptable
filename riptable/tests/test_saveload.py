...
class SaveLoad_Test(unittest.TestCase):
    ...
    def test_save_categorical(self):
        ...
        # enum
        cmap = Categorical(
            [10, 20, 30, 40, 50], {10: "a", 20: "b", 30: "c", 40: "d", 50: "e"}
        )
    ...
    def test_sharedmem_save(self):
        ...
        if sys.platform != "linux":
            ...
            self.assertFalse(
                os.path.exists(r"riptable/tests/temp/temp_save_shared.sds")
            )

            # after loading, compare dataset to original
            ds2 = Dataset.load(
                r"riptable/tests/temp/temp_save_shared", share="test_save_shared"
            )
            ...
            ds.save(
                r"Z:::/riptable/tests/temp/temp_save_shared", share="test_save_shared"
            )
            ds2 = Dataset.load(
                r"Y::::::/differentfilepath/samename/temp_save_shared",
                share="test_save_shared",
            )
            for k, v in ds2.items():
                self.assertTrue(bool(np.all(v == ds[k])))
    ...
    def test_uncompressed_save(self):
        ds = Dataset(
            {"col_" + str(i): zeros(100_000, dtype=np.int32) for i in range(50)}
        )
    ...
    def test_meta_tuples(self):
        ...
        ds_tups = decompress_dataset_internal(r"riptable/tests/temp/tempsave_ds.sds")[
            0
        ][2]
        ...
        st_tups = decompress_dataset_internal(r"riptable/tests/temp/tempsave_st.sds")[
            0
        ][2]
    ...
    def test_meta_tuples_new(self):
        ...
        _, _, tups, _ = decompress_dataset_internal(
            r"riptable/tests/temp/tempsave_st/_root.sds"
        )[0]
    ...
    def test_stack_missing_string(self):
        ds1 = Dataset(
            {"col1": arange(5), "strings": FA(["a", "b", "c", "d", "e"], unicode=True)}
        )
    ...
    def test_stack_dir(self):
        ...
        ds2 = Dataset({"col_" + str(i): np.random.rand(5) for i in range(5)})
        files1 = [
            r"riptable/tests/temp/dir1/ds1.sds",
            r"riptable/tests/temp/dir2/ds1.sds",
        ]
        files2 = [
            r"riptable/tests/temp/dir1/ds2.sds",
            r"riptable/tests/temp/dir2/ds2.sds",
        ]
        for f in files1:
            ds1.save(f)
        for f in files2:
    ...
    def test_stack_upcast(self):
        ...
        for dt in int_dt + uint_dt + flt_dt:
            ...
            self.assertTrue(
                bool(np.all(ds.nums == correct)),
                msg=f"Failure in stacking: float64 and {np.dtype(dt).name}",
            )
            self.assertTrue(
                ds.nums.dtype == np.float64,
                msg=f"Failure in stacking: float64 and {np.dtype(dt).name}",
            )
        ...
        for dt in int_dt[:2] + uint_dt[:2]:
            ...
            self.assertTrue(
                bool(np.all(ds.nums == correct)),
                msg=f"Failure in stacking: float32 and {np.dtype(dt).name}",
            )
            self.assertTrue(
                ds.nums.dtype == np.float32,
                msg=f"Failure in stacking: float32 and {np.dtype(dt).name}",
            )
        for dt in int_dt[2:] + uint_dt[2:]:
            files = [flt32, r"riptable/tests/temp/upcast/" + np.dtype(dt).name]
        ...
        for dt in int_dt + uint_dt[:-1]:
            ...
            self.assertTrue(
                bool(np.all(ds.nums == correct)),
                msg=f"Failure in stacking: int64 and {np.dtype(dt).name}",
            )
            self.assertTrue(
                ds.nums.dtype == np.int64,
                msg=f"Failure in stacking: int64 and {np.dtype(dt).name}",
            )
        for dt in flt_dt:
            files = [i64, r"riptable/tests/temp/upcast/" + np.dtype(dt).name]
        ...
        for dt in int_dt[:-1] + uint_dt[:2]:
            ...
            self.assertTrue(
                bool(np.all(ds.nums == correct)),
                msg=f"Failure in stacking: int32 and {np.dtype(dt).name}",
            )
            self.assertTrue(
                ds.nums.dtype == np.int32,
                msg=f"Failure in stacking: int32 and {np.dtype(dt).name}",
            )
        for dt in flt_dt:
            files = [i32, r"riptable/tests/temp/upcast/" + np.dtype(dt).name]
        ...
        for dt in int_dt[:2] + uint_dt[:1]:
            ...
            self.assertTrue(
                bool(np.all(ds.nums == correct)),
                msg=f"Failure in stacking: int16 and {np.dtype(dt).name}",
            )
            self.assertTrue(
                ds.nums.dtype == np.int16,
                msg=f"Failure in stacking: int16 and {np.dtype(dt).name}",
            )
        for dt in flt_dt:
            files = [i16, r"riptable/tests/temp/upcast/" + np.dtype(dt).name]
        ...
        for dt in int_dt + flt_dt:
            ...
            self.assertTrue(
                bool(np.all(ds.nums == correct)),
                msg=f"Failure in stacking: int8 and {np.dtype(dt).name}",
            )
            self.assertTrue(
                ds.nums.dtype == dt,
                msg=f"Failure in stacking: int8 and {np.dtype(dt).name}",
        ...
        for dt in uint_dt:
            ...
            self.assertTrue(
                bool(np.all(ds.nums == correct)),
                msg=f"Failure in stacking: uint64 and {np.dtype(dt).name}",
            )
            self.assertTrue(
                ds.nums.dtype == np.uint64,
                msg=f"Failure in stacking: uint64 and {np.dtype(dt).name}",
            )
        for dt in int_dt[:-1]:
            files = [ui64, r"riptable/tests/temp/upcast/" + np.dtype(dt).name]
        ...
        for dt in int_dt[:-2]:
            ...
            self.assertTrue(
                bool(np.all(ds.nums == correct)),
                msg=f"Failure in stacking: uint32 and {np.dtype(dt).name}",
            )
            self.assertTrue(
                ds.nums.dtype == np.int64,
                msg=f"Failure in stacking: uint32 and {np.dtype(dt).name}",
        ...
        for dt in int_dt[:-3]:
            ...
            self.assertTrue(
                bool(np.all(ds.nums == correct)),
                msg=f"Failure in stacking: uint16 and {np.dtype(dt).name}",
            )
            self.assertTrue(
                ds.nums.dtype == np.int32,
                msg=f"Failure in stacking: uint16 and {np.dtype(dt).name}",
        ...
        self.assertTrue(
            bool(np.all(ds.nums == correct)),
            msg=f"Failed to upcast int32/uint32 -> int64",
        )
        self.assertTrue(
            ds.nums.dtype == np.int64, msg="Failed to upcast int32/uint32 -> int64"
        )
        ...
        self.assertTrue(
            bool(np.all(ds.nums == correct)),
            msg=f"Failed to upcast int16/uint16 -> int32",
        )
        self.assertTrue(
            ds.nums.dtype == np.int32, msg="Failed to upcast int16/uint16 -> int32"
        )
        ...
        self.assertTrue(
            bool(np.all(ds.nums == correct)),
            msg=f"Failed to upcast int8/uint8 -> int16",
        )
        self.assertTrue(
            ds.nums.dtype == np.int16, msg="Failed to upcast int8/uint8 -> int16"
        )
    ...
    def test_load_list(self):
        ...
        for i, item in enumerate(reload):
            self.assertTrue(
                isinstance(item, types[i]), msg=f"expected {types[i]} got {type(item)}"
            )
    ...
    def test_load_list_include(self):
        ...
        files = [
            r"riptable/tests/temp/tempsave/ds1",
            r"riptable/tests/temp/tempsave/ds2",
        ]
        inc = ["col_2", "col_3"]
        ...
        ds = Dataset(
            {
                ...
                "TSPAN": TimeSpan(
                    np.random.randint(
                        0, 1_000_000_000 * 60 * 60 * 24, 1000, dtype=np.int64
                    )
                ),
    ...
    def test_include_categorical(self):
        ds = Dataset(
            {"arr": arange(5), "cat_col": Categorical(["a", "a", "b", "c", "a"])}
        )
    ...
    def test_sds_flatten(self):
        ...
        try:
            os.rename(
                r"riptable/tests/temp/root_temp/st3/garbagefile.sds",
                r"riptable/tests/temp/root_temp/st3/garbagefile",
            )
        except FileExistsError:
            pass
        ...
        s4 = load_sds(
            [r"riptable/tests/temp/st4.sds", r"riptable/tests/temp/st4.sds"], stack=True
        )
        ...
        ds = Dataset(
            {
                "int8": np.random.randint(
                    low=-128, high=127, size=(test_len), dtype=np.int8
                ),
                "uint8": np.random.randint(
                    low=0, high=255, size=(test_len), dtype=np.uint8
                ),
                "int16": np.random.randint(
                    low=-32768, high=32767, size=(test_len), dtype=np.int16
                ),
                "uint16": np.random.randint(
                    low=0, high=65535, size=(test_len), dtype=np.uint16
                ),
                "int32": np.random.randint(
                    low=-2147483648, high=2147483647, size=(test_len), dtype=np.int32
                ),
                "uint32": np.random.randint(
                    low=0, high=4294967295, size=(test_len), dtype=np.uint32
                ),
                ...
                "uint64": np.random.randint(
                    low=0, high=18446744073709551615, size=(test_len), dtype=np.uint64
                ),
                "float32": np.random.rand(test_len).astype(np.float32) * 10e9,
                "float64": np.random.rand(test_len).astype(np.float64) * 10e-9,
                "unicode_str": FastArray(
                    np.random.choice(["a", "b", "c"], test_len), unicode=True
                ),
...
@pytest.mark.parametrize(
    ...
    [
        1,
        pytest.param(
            0, marks=pytest.mark.xfail(reason="Same as reason for param `2` below")
        ),
        pytest.param(
            2,
            marks=pytest.mark.xfail(
...
def test_sds_stack(data, stack, stack_count, tmpdir):
    ...
    for k, v in data.items():
        ...
        if isinstance(exp, Categorical):
            ...
            pytest.fail(
                f"{fn}: assertions not implemented for data type {type(exp)}\n"
                + err_msg
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            Struct({"d": Dataset({})}),
            marks=pytest.mark.xfail(
                reason="_sds_from_tree: 'NoneType' object cannot be interpreted as an integer"
            ),
        ),
        pytest.param(
            Struct({"m": Multiset()}),
