...
def test_merge_extract_on_columns(
    ...
    if is_error:
        with pytest.raises(ValueError):
            rt.rt_merge._extract_on_columns(
                on, side_on, for_left, "on", is_optional=False
            )

    else:
        result = rt.rt_merge._extract_on_columns(
            on, side_on, for_left, "on", is_optional=False
        )
...
class TestDataset:
    ...
    def sql_semantics2() -> Tuple[rt.Dataset, rt.Dataset]:
        ...
        bar.col1 = rt.FA(
            [10, 10, 8, inv, inv, inv, inv, 5, 5, 14, 5, -15], dtype=np.int32
        )
        bar.col2 = rt.FA(
            [4, inv, inv, 3, inv, inv, 1, 5, inv, 9, 5, 13], dtype=np.int32
        )
        bar.strcol = rt.FA(
            [
                b"Chestnut",
...
def test_merge2_result_matches_computed_fancyindex(
    ...
    assert JoinIndices.result_rowcount(
        join_indices.left_index, left.get_nrows()
    ...
    assert len(merge_result) == JoinIndices.result_rowcount(
        join_indices.left_index, left.get_nrows()
    )
...
def test_merge_create_fancy_indices_with_invalids(
    ...
    # to transform the columns from the left and right Datasets, all the
    # transformed columns have the same length.
    left_fancyindex_len = JoinIndices.result_rowcount(
        join_indices.left_index, left.get_nrows()
    )
    assert left_fancyindex_len == JoinIndices.result_rowcount(
    ...
    def create_and_verify_join_indices(
        ...
        left_filt_fancyindex_len = JoinIndices.result_rowcount(
            filt_join_indices.left_index, len(left_grp.ikey)
        )
...
def test_merge2_multikey_join_with_cats():
    ...
    right_ds["PurchaseDate"] = rt.Cat(
        rt.Date(
            np.repeat(
                np.array(["2019-06-19", "2020-02-10", "2020-03-17", "2019-12-01"]),
                repeats=4,
            )
        )
    )
...
def test_merge2_sql_semantics_outerjoin_single():
    ...
    assert_array_equal(
        result.id_foo,
        rt.FA(
            [1, 1, 1, 2, 2, 2, 3, 4, 5, 5, 6, 7, 8, inv, inv, inv, inv, inv, inv],
            dtype=np.int32,
        ),
    )
    assert_array_equal(
    ...
    assert_array_equal(
        result.id_bar,
        rt.FA(
            [8, 9, 11, 8, 9, 11, 3, inv, 1, 2, inv, inv, inv, 4, 5, 6, 7, 10, 12],
            dtype=np.int32,
        ),
    )
    assert_array_equal(
...
def test_merge2_sql_semantics_outerjoin_single_keep_Nonefirst():
    ...
    assert_array_equal(
        result.col1,
        rt.FA([5, 5, 8, inv, 10, inv, -1, 11, inv, 14, -15], dtype=np.int32),
    )

    # Cols from the left Dataset.
    assert_array_equal(
        result.id_foo, rt.FA([1, 2, 3, 4, 5, 6, 7, 8, inv, inv, inv], dtype=np.int32)
    )
    assert_array_equal(
        result.col2_foo,
        rt.FA([inv, 5, inv, 1, 1, 4, 22, 9, inv, inv, inv], dtype=np.int32),
    )
    assert_array_equal(
        result.team_name,
        rt.FA(
    ...
    assert_array_equal(
        result.id_bar,
        rt.FA([8, 8, 3, inv, 1, inv, inv, inv, 4, 10, 12], dtype=np.int32),
    )
    assert_array_equal(
        result.col2_bar,
        rt.FA([5, 5, inv, inv, 4, inv, inv, inv, 3, 9, 13], dtype=np.int32),
    )
    assert_array_equal(
        result.strcol,
        rt.FA(
...
def test_merge2_sql_semantics_outerjoin_single_keep_Nonelast():
    ...
    assert_array_equal(
        result.id_bar,
        rt.FA([11, 11, 3, inv, 2, inv, inv, inv, 7, 10, 12], dtype=np.int32),
    )
    assert_array_equal(
        result.col2_bar,
        rt.FA([5, 5, inv, inv, inv, inv, inv, inv, 1, 9, 13], dtype=np.int32),
    )
    assert_array_equal(
        result.strcol,
        rt.FA(
...
def test_merge2_sql_semantics_outerjoin_single_keep_firstNone():
    ...
    assert_array_equal(
        result.col1,
        rt.FA(
            [5, 5, 5, 8, inv, 10, 10, -1, 11, inv, inv, inv, inv, 14, -15],
            dtype=np.int32,
        ),
    )
    ...
    assert_array_equal(
        result.id_foo,
        rt.FA(
            [1, 1, 1, 3, 4, 5, 5, 7, 8, inv, inv, inv, inv, inv, inv], dtype=np.int32
        ),
    )
    assert_array_equal(
        result.col2_foo,
        rt.FA(
            [inv, inv, inv, inv, 1, 1, 1, 22, 9, inv, inv, inv, inv, inv, inv],
            dtype=np.int32,
        ),
    )
    assert_array_equal(
    ...
    assert_array_equal(
        result.id_bar,
        rt.FA([8, 9, 11, 3, inv, 1, 2, inv, inv, 4, 5, 6, 7, 10, 12], dtype=np.int32),
    )
    assert_array_equal(
        result.col2_bar,
        rt.FA(
            [5, 5, 5, 8, inv, 10, 10, inv, inv, inv, inv, inv, inv, 14, -15],
            dtype=np.int32,
        ),
    )
    assert_array_equal(
...
def test_merge2_sql_semantics_outerjoin_multi():
    ...
    assert_array_equal(
        result.col2,
        rt.FA(
            [inv, 5, 5, inv, 1, 1, 4, 22, 9, 4, inv, inv, 3, inv, inv, 1, inv, 9, 13],
            dtype=np.int32,
        ),
    )
    ...
    assert_array_equal(
        result.id_bar,
        rt.FA(
            [inv, 8, 11, inv, inv, inv, inv, inv, inv, 1, 2, 3, 4, 5, 6, 7, 9, 10, 12],
            dtype=np.int32,
        ),
    )
    assert_array_equal(
...
def test_merge2_sql_semantics_outerjoin_multi_keep_Nonefirst():
    ...
    assert_array_equal(
        result.col1,
        rt.FA(
            [5, 5, 8, inv, 10, inv, -1, 11, 10, 10, 8, inv, inv, inv, 5, 14, -15],
            dtype=np.int32,
        ),
    )
    assert_array_equal(
        result.col2,
        rt.FA(
            [inv, 5, inv, 1, 1, 4, 22, 9, 4, inv, inv, 3, inv, 1, inv, 9, 13],
            dtype=np.int32,
        ),
    )
    ...
    assert_array_equal(
        result.id_foo,
        rt.FA(
            [1, 2, 3, 4, 5, 6, 7, 8, inv, inv, inv, inv, inv, inv, inv, inv, inv],
            dtype=np.int32,
        ),
    )
    assert_array_equal(
    ...
    assert_array_equal(
        result.id_bar,
        rt.FA(
            [inv, 8, inv, inv, inv, inv, inv, inv, 1, 2, 3, 4, 5, 7, 9, 10, 12],
            dtype=np.int32,
        ),
    )
    assert_array_equal(
...
def test_merge2_sql_semantics_outerjoin_multi_keep_Nonelast():
    ...
    assert_array_equal(
        result.id_bar,
        rt.FA(
            [inv, 11, inv, inv, inv, inv, inv, inv, 1, 2, 3, 4, 6, 7, 9, 10, 12],
            dtype=np.int32,
        ),
    )
    assert_array_equal(
...
def test_merge2_sql_semantics_leftjoin_multi_keep_firstlast():
    ...
    # Intersection cols (the 'on' cols)
    assert_array_equal(
        result.col1, rt.FA([5, 5, 8, inv, 10, inv, -1, 11], dtype=np.int32)
    )
    assert_array_equal(
        result.col2, rt.FA([inv, 5, inv, 1, 1, 4, 22, 9], dtype=np.int32)
    )
    ...
    # Cols from the right Dataset.
    assert_array_equal(
        result.id_bar, rt.FA([inv, 11, inv, inv, inv, inv, inv, inv], dtype=np.int32)
    )
    assert_array_equal(
...
def test_merge2_sql_semantics_rightjoin_multi_keep_firstlast():
    ...
    # Intersection cols (the 'on' cols)
    assert_array_equal(
        result.col1, rt.FA([10, 10, 8, inv, inv, inv, 5, 14, 5, -15], dtype=np.int32)
    )
    assert_array_equal(
        result.col2, rt.FA([4, inv, inv, 3, inv, 1, inv, 9, 5, 13], dtype=np.int32)
    )

    # Cols from the left Dataset.
    assert_array_equal(
        result.id_foo,
        rt.FA([inv, inv, inv, inv, inv, inv, inv, inv, 2, inv], dtype=np.int32),
    )
    assert_array_equal(
        result.team_name,
    ...
    # Cols from the right Dataset.
    assert_array_equal(
        result.id_bar, rt.FA([1, 2, 3, 4, 6, 7, 9, 10, 11, 12], dtype=np.int32)
    )
    assert_array_equal(
        result.strcol,
        rt.FA(
...
@pytest.mark.parametrize(
    "keep_left", [None, pytest.param("first"), pytest.param("last")]
)
@pytest.mark.parametrize(
    "keep_right", [None, pytest.param("first"), pytest.param("last")]
)
def test_merge2_with_unused_categories(how, keep_left, keep_right):
    x = rt.Dataset(
        {
            "k1": rt.Categorical(
                ["a", "b", "c", "b", "a", "a", "c"], ["a", "b", "c", "d"]
            ),
            "a": [1, 2, 3, 4, 5, 6, 7],
        }
    )
...
def test_lookup_copy_when_right_already_unique():
    ...
    left_ds = rt.Dataset()
    left_ds["Symbol"] = rt.FA(
        ["AAPL", "AMZN", "AA", "AAN", "AAPL", "AMZN", "AAP", "AMZN"]
    )
...
def test_merge_lookup_inplace_single():
    ...
    foo.merge_lookup(
        bar, on=("col1", "col1"), suffix="_bar", keep="first", inplace=True
    )
...
class MergeAsofTest(unittest.TestCase):
    ...
    def test_merge_asof_categorical_and_string_keys(self):
        ...
        for key in ds.keys():
            assert_array_equal(
                ds[key],
                target[key],
                err_msg=f"Column '{key}' differs between the actual and expected.",
            )
    ...
    def test_merge_asof_nearest(self):
        ...
        merged = ds1.merge_asof(
            ds2, on="Time", direction="nearest", matched_on="Time_y"
        )
