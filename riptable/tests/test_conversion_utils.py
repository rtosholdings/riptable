...
from riptable import Categorical, Dataset
...
class Conversion_Utility_Test(unittest.TestCase):
    ...
    def test_as_matrix_metadata(self):
        ...
        self.assertTrue(
            (numpy.abs(X[:, 0] - numpy.array([0.0, 1.0, 0.0, 2.0, 2.0])) < error_tol).all(), msg=f"got {X[:, 0]}"
        )
        self.assertTrue((numpy.abs(X[:, 2] - numpy.array([0, 0, 1, 0, 2])) < error_tol).all(), msg=f"got {X[:, 2]}")
        self.assertTrue(
    ...
    def test_as_matrix_int(self):
        error_tol = 0.00001
        ds = Dataset({_k: list(range(_i * 10, (_i + 1) * 10)) for _i, _k in enumerate("ABCDEFGHIJKLMNOP")})
        X, _ = dataset_as_matrix(ds)
        self.assertIsInstance(X, numpy.ndarray)
        self.assertEqual(X.shape[0], ds.shape[0])
        self.assertEqual(X.shape[1], ds.shape[1])  # we may break this later
    ...
    def test_numpy_array_to_dict(self):
        ...
        arr = numpy.array([(1.0, "Q"), (-3.0, "Z")], dtype=[("x", numpy.float64), ("y", "S1")])
        dd = numpy_array_to_dict(arr)
        self.assertEqual(list(dd), ["x", "y"])
        self.assertTrue((dd["x"] == numpy.array([1.0, -3.0], dtype=numpy.float64)).all())
