...
@pytest.mark.parametrize(
    "typ,arrays",
    [
        pytest.param(DateTimeNano, (DateTimeNano([], from_tz="NYC", to_tz="NYC"),),),
        (
            DateTimeNano,
            (
                DateTimeNano([], from_tz="NYC", to_tz="NYC"),
                DateTimeNano(
                    ["20191126 10:00:43.232329062"], from_tz="NYC", to_tz="NYC"
                ),
            ),
        ),
        (
            ...
            (
                DateTimeNano(
                    ["20191126 14:04:31.421071000",], from_tz="NYC", to_tz="NYC"
                ),
                DateTimeNano([], from_tz="NYC", to_tz="NYC"),
            ),
        ),
        (
            ...
            (
                ...
                DateTimeNano(
                    ["20191126 10:00:43.232329062", "20191126 14:04:31.421071000",],
                    from_tz="NYC",
                    to_tz="NYC",
                ),
                DateTimeNano(
...
def test_concatenate_preserves_datetime_type_regression(typ, arrays):
    ...
    assert all(
        [isinstance(a, typ) for a in arrays]
    ), f"{fn}: expected tuple of arrays to be the same type"
