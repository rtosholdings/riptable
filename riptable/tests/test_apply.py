...
class transform_apply_test(unittest.TestCase):
        ...
        for f in reduce_funcs:
            ...
            for gb_rownum, gb_unique in enumerate(["a", "b", "c", "d", "e"]):
                ...
                for i in range(5):
                    ...
                    self.assertAlmostEqual(ds_item, gb_item, msg=f"func:{f} {ds_item} {gb_item}")
    ...
    def test_apply_filter_multikey(self):
        ...
        ds_cat = Dataset({GROUPBY_KEY_PREFIX: Categorical([st_arr, int_arr]), "data": data})
    ...
    def test_apply_ema(self):
        ...
        ema_result = ds.gb("sym").apply_nonreduce(get_ema_userfunc2, (ds.org,), decay_rate=decay)
        ...
        # create another org
        ds.org2 = ds.org + 1
        ema_result = ds.gb("sym")["org", "org2"].apply_nonreduce(get_ema_userfunc2, (ds.time,), decay_rate=decay)
        self.assertTrue(sum(correct - ema_result["org"]) < 0.01)

        ema_result = ds.sym.apply_nonreduce(get_ema_userfunc2, [ds.org, ds.org2], (ds.time,), decay_rate=decay)
        self.assertTrue(sum(correct - ema_result["org"]) < 0.01)
    ...
    def test_apply_tuple(self):
        ...
        result = ds.symbol.apply_reduce(lambda x, y: np.sum(np.minimum(x, y)), (ds.data, ds.data))[0]
    ...
    def test_apply_gb_vs_cat(self):
        ...
        newds = c.count()
        newds["dollars_exec"] = c.apply_reduce(lambda x, y: (x * y).nansum(), ds.TradeSize, (ds.TradePrice,))
        newds
        ...
        r1 = ds.groupby(["Symbol", "Exchange"])["TradeSize"].apply_reduce(lambda x: len(x))[0]
        r2 = ds.groupby(["Symbol", "Exchange"]).count()[0]
        self.assertTrue(arr_eq(r1, r2))

    def test_apply_computable(self):
        ds = Dataset({"a": Cat(list("AABB")), "b": ["foo", "bar", "baz", "bazz"]}, unicode=True)
        ...
        ds["d"] = ds.a.copy()
        ds.c = arange(4)
        result = ds["a"].apply_nonreduce(FA.shift, ds[["b", "c", "d"]], computable=False)
        self.assertTrue(result["d"][1] == "A")

    def test_apply_dups(self):
        ds = Dataset({"a": FA(["B", "B"]), "b": FA(["Bb", "Bb"]), "i": FA([1, 1])})
        c = Cat([ds.a, ds.b])
        result = c.apply(np.sum, ds.i)
        self.assertEqual(result._ncols, 3)

        ds = Dataset({"a": FA(["B", "D"]), "b": FA(["Bb", "Bb"]), "i": FA([1, 1])})
        c = Cat([ds.a, ds.b])
        result = c.apply(np.sum, ds.i)
        self.assertEqual(result._ncols, 4)
