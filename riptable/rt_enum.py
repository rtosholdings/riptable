...
class MATH_OPERATION(IntEnum):
    ...
    SUBDATETIMES = (
        102,
    )  # will check both sides for INV/ZEROS input:int64/int32 (returns double)
    # SUBDATETIMESRIGHT = 103,  # will check right hand side for ZEROS input:int64/int32 (returns double)
    SUBDATES = (
        103,
    )  # will check both sides for INV/ZEROS same dtype in/out (int32/int64)
...
class TypeRegister:
    ...
    def is_computable(cls, other):
        if not (
            other.dtype.char in NumpyCharTypes.Noncomputable
            or isinstance(other, cls.Categorical)
        ):
            return True
        return False
    ...
    def is_datelike(cls, arr: np.ndarray):
        ...
        if isinstance(
            arr,
            (
                TypeRegister.DateTimeNano,
                TypeRegister.TimeSpan,
                TypeRegister.Date,
                TypeRegister.DateSpan,
            ),
        ):
            result = True
        return result
