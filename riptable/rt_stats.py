...
def linear_spline(X0, Y0, knots, display=True):
    ...
    output_notebook()
    # create a new plot
    p = figure(tools="pan,box_zoom,reset,save", title="example", x_axis_label="sections", y_axis_label="particles")
...
def plotPrediction(X, Yhat, Y, N, lb=None, ub=None):
    ...
    goodFilt = (
        np.isfinite(X) & np.isfinite(Y) & (X <= upperBound) & (X >= lowerBound) & np.isfinite(Yhat) & np.isfinite(Y)
    )
