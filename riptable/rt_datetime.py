...
class DateTimeBase(FastArray):
    ...
    def display_item(self, utcnano):
        raise NotImplementedError(
            f"DateTimeBase subclasses need to override this method."
        )
    ...
    def _funnel_mathops(self, funcname, value):
        ...
        result = self._build_mathops_result(
            value, result, inv_mask, other_inv_mask, return_type
        )
        return result
...
class TimeStampBase:
    ...
    def _year(self, arr, fix_dst=False):
        ...
        result = (
            self._year_splits.searchsorted(arr, side="right").astype(
                np.int32, copy=False
            )
            + 1969
        )
    ...
    def _month(self, arr=None, fix_dst=False):
        ...
        putmask(
            smonth,
            maskleap,
            self._yearday_splits_leap.searchsorted(startyear, side="right"),
        )
...
class DateBase(FastArray):
        ...
        if isinstance(self, np.ndarray):
            return np.asarray(
                [
                    ...
                    for timestamp in self._fa * SECONDS_PER_DAY
                ],
                dtype=dtype,
            )
        else:
    ...
    def _funnel_mathops(self, funcname, value):
        ...
        if funcname in self.forbidden_mathops:
            raise TypeError(
                f"Cannot perform {funcname} on {self.__class__.__name__} object."
            )
    ...
    # ------------------------------------------------------------
    def _build_mathops_result(
        self, value, result, inv_mask, other_inv_mask, return_type
    ):
        ...
        result = _apply_inv_mask(
            self,
            result,
            fillval=self.NAN_DATE,
            arr1_inv_mask=inv_mask,
...
class Date(DateBase, TimeStampBase):
        ...
        if isinstance(arr, np.ndarray):
            ...
            if not isinstance(arr, Date):
                ...
                if isinstance(arr, TypeRegister.Categorical):
                    try:
                        ...
                        if cats.dtype.char in ("U", "S"):
                            cats = cls._convert_datestring(cats).astype(
                                np.int32, copy=False
                            )
                        ...
                        raise TypeError(
                            f"Could not re-expand categorical to array in mode {arr.category_mode.name}"
                        )
                ...
                if from_matlab:
                    ...
                    raise TypeError(
                        f"Could not initialize Date object with array of type {arr.dtype}."
                    )
    ...
    def format_date_num(date_num, itemformat):
        if (
            date_num == DateBase.NAN_DATE
            or date_num == INVALID_DICT[np.dtype(np.int32).num]
        ):
            return "Inv"
        ...
        try:
            ...
            raise ValueError(
                f"Date number {date_num} is not a valid value for Date() object."
            )
        return timestr
    ...
    def fill_invalid(self, shape=None, dtype=None, inplace=True):
        arr = self._fill_invalid_internal(
            shape=shape, dtype=self.dtype, fill_val=self.NAN_DATE, inplace=inplace
        )
        if arr is None:
            return
        return Date(arr)
    ...
    def yyyymmdd(self):
        return DateTimeNano(
            self._fa * NANOS_PER_DAY, from_tz="GMT", to_tz="GMT"
        ).yyyymmdd
    ...
    def day_of_month(self):
        ...
        leapmask = (year % 4) == 0
        startmonth_idx_leap = (
            self._yearday_splits_leap.searchsorted(startyear[leapmask], side="right")
            - 1
        )
        monthtime[leapmask] = (
            startyear[leapmask] - self._yearday_splits_leap[startmonth_idx_leap]
        )
    ...
    def range(cls, start, end=None, days=None, step=1, format=None, closed=None):
        ...
        if end is None:
            ...
            if not isinstance(end, (str, bytes)):
                raise TypeError(
                    f"End date must be string or integer. Got {type(start)}"
                )
        ...
        if days is None and step == 1:
            ...
            if closed is None:
                ...
                raise ValueError(
                    f'Closed has to be either "left", "right" or None. Got {closed}'
                )
    ...
    def _date_compare_check(self, funcname, other):
        ...
        if isinstance(other, (DateSpan, TimeSpan, DateSpanScalar, TimeSpanScalar)):
            raise TypeError(
                f"Cannot perform {funcname} comparison operation between {type(self)} and {type(other)}."
            )
    ...
    def __sub__(self, value):
        ...
        if isinstance(value, Date):
            ...
            caller = DateTimeNano(
                self._fa * NANOS_PER_DAY, from_tz=value._timezone._from_tz
            )
            return caller - value
    ...
    def _check_mathops_nano(self, funcname, value, other_inv_mask, return_type, caller):
        ...
        if isinstance(value, TimeSpan):
            ...
            if funcname in ("__add__", "__iadd__", "__isub__"):
                raise TypeError(
                    f"Cannot perform addition between Date and DateTimeNano"
                )
...
class DateSpan(DateBase):
    ...
    def fill_invalid(self, shape=None, dtype=None, inplace=True):
        arr = self._fill_invalid_internal(
            shape=shape, dtype=self.dtype, inplace=inplace
        )
        if arr is None:
            return
        return DateSpan(arr)
    ...
    def _check_mathops_nano(self, funcname, value, other_inv_mask, return_type, caller):
        ...
        if isinstance(value, TimeSpan):
            ...
            if funcname in ("__sub__", "__isub__"):
                raise TypeError(
                    f"Cannot perform {funcname} operation between DateSpan and DateTimeNano"
                )
            return_type = DateTimeNano
            other_inv_mask = value.isnan()
            caller = self._fa * NANOS_PER_DAY
    ...
    def _check_mathops(self, funcname, value):
        ...
        if isinstance(value, Date):
            if funcname in ("__sub__", "__isub__"):
                raise TypeError(
                    f"Cannot perform {funcname} operation between DateSpan and Date"
                )
            return_type = Date
            other_inv_mask = value.isnan()
...
# ------------------------------------------------------------
def DateTimeUTC(
    arr, to_tz="NYC", from_matlab=False, format=None, start_date=None, gmt=None
):
...
class DateTimeCommon:
    ...
    def date(self):
        ...
        result = DateTimeNano(
            arr, from_tz=self._timezone._to_tz, to_tz=self._timezone._to_tz
        )
    ...
    def yyyymmdd(self):
        ...
        # fix up the leapyears with a different yearday split table
        leapmask = (year % 4) == 0
        monthnum_leap = self._yearday_splits_leap.searchsorted(
            startyear[leapmask], side="right"
        )
    ...
    def microsecond_span(self):
        ...
        return self._time_fraction(
            NANOS_PER_MILLISECOND, NANOS_PER_MICROSECOND, span=True
        )
    ...
    def _build_mathops_result(
        self, other, funcname, call_super, other_inv_mask, inplace, op, return_type
    ):
        """
        ...
        if call_super:
            ...
            result = _apply_inv_mask(
                input1,
                result,
                fillval=DateTimeBase.NAN_TIME,
                arr1_inv_mask=input1_mask,
                arr2_inv_mask=other_inv_mask,
            )
        else:
            if inplace:
    ...
    def strftime(self, format, dtype="O"):
        ...
        if to_tz in ["GMT", "UTC"]:
            ...
            if isinstance(in_seconds, np.ndarray):
                return np.asarray(
                    [
                        dt.fromtimestamp(timestamp, localzone).strftime(format)
                        for timestamp in in_seconds
                    ],
                    dtype=dtype,
                )
...
class DateTimeNano(DateTimeBase, TimeStampBase, DateTimeCommon):
            ...
            if isinstance(arr, np.ndarray):
                ...
                if arr.dtype.char == "O":
                    ...
                    if isinstance(arr[0], dt):
                        ...
                        arr = np.array(
                            [t.isoformat() for t in arr], dtype="datetime64[ns]"
                        )
                ...
                if arr.dtype.char in "US":
                    ...
                    if format is not None:
                        instance = strptime_to_nano(
                            arr, format, from_tz=from_tz, to_tz=to_tz
                        )
                    else:
                        # otherwise assume ISO-8601 format
                        instance = datetimestring_to_nano(
                            arr, from_tz=from_tz, to_tz=to_tz
                        )
                ...
                # don't allow timespan arrays without start date
                    ...
                    raise TypeError(
                        f"Cannot create DateTimeNano from TimeSpan array unless start_date is provided."
                    )
                    ...
                    raise TypeError(
                        f"Cannot create DateTimeNano object from {arr.dtype}"
                    )
                ...
                raise TypeError(
                    f"Cannot initialize DateTimeNano with type {type(arr)}, must be list or array."
                )
    ...
    def _parse_item_format(cls, itemformat):
        ...
        if itemformat.length == DisplayLength.Short:
            ...
            raise ValueError(
                f"Don't know how to interpret display length: {itemformat.length.name}"
            )
    ...
    def format_nano_time(utcnano, itemformat):
        ...
        try:
            ...
            raise ValueError(
                f"DateTime: the utc nano value {utcnano!r} for {timezone.utc!r} is not valid."
            )
    ...
    # ------------------------------------------------------------
    @classmethod
    def _load_from_sds_meta_data(
        cls, name, arr, cols, meta, tups: Optional[list] = None
    ):
        """
        Note
        ----
    ...
    def __repr__(self, verbose=False):
        ...
        repr_strings.append(
            self.get_classname() + "([" + self._build_string() + "]" + tz_string + ")"
        )
    ...
    def cut_time(
        ...
        if is_already_list:
            ...
            if nyc and (start_time is not None and end_time is not None):
                raise ValueError(
                    "If nyc is True then you can't set both start and end time bounds"
                )
        ...
        return TypeRegister.Categorical(
            cat, bucket_cut_labels, base_index=1, ordered=False
        )

    # -------------------------------------------------------------
    def fill_invalid(self, shape=None, dtype=None, inplace=True):
        arr = self._fill_invalid_internal(
            shape=shape, dtype=self.dtype, fill_val=self.NAN_TIME, inplace=inplace
        )
        if arr is None:
            return
    ...
    def _datetimenano_compare_check(self, funcname, other):
        ...
        if isinstance(other, (DateTimeNano, DateTimeNanoScalar)):
            ...
            other = DateTimeNano(
                other._fa * NANOS_PER_DAY,
                from_tz=self._timezone._to_tz,
                to_tz=self._timezone._to_tz,
            )
    ...
    def min(self, **kwargs):
        ...
        """
        return DateTimeNano(
            [self._fa.min()], from_tz="GMT", to_tz=self._timezone._to_tz
        )
    ...
    def max(self, **kwargs):
        ...
        """
        return DateTimeNano(
            [self._fa.max()], from_tz="GMT", to_tz=self._timezone._to_tz
        )
    ...
    def __add__(self, other, inplace=False):
        ...
        if not isinstance(other, np.ndarray) and not isinstance(
            ...
            if isinstance(other, (DateTimeNano, DateTimeNanoScalar)):
                raise TypeError(
                    f"Cannot add two objects {type(self)} and {type(other)}"
                )
            elif isinstance(other, (Date, DateScalar)):
                raise TypeError(
                    f"Cannot add two objects {type(self)} and {type(other)}"
        ...
        return self._build_mathops_result(
            other, funcname, call_super, other_inv_mask, inplace, op, return_type
        )
    ...
    def __rsub__(self, other):
        if isinstance(other, (Date, DateScalar)):
            ...
            raise TypeError(
                f"DateTimeNano can only be subtracted from DateTimeNano or Date."
            )

    # -------------------------------------------------------------
    def __isub__(self, other):
        warnings.warn(
            f"Currently allowing inplace operation __isub__ on DateTimeNano. May change in the future."
        )
    ...
    def __sub__(self, other, inplace=False):
        ...
        if not isinstance(other, np.ndarray) and not isinstance(
            ...
            if isinstance(other, (DateTimeNano, DateTimeNanoScalar)):
                ...
                if inplace:
                    raise TypeError(
                        f"__sub__ returns TimeSpan. Cannot perform inplace."
                    )
                op = MATH_OPERATION.SUBDATETIMES
    ...
    def _guard_math_op(self, value, op_name):
        if isinstance(value, DateTimeBase):
            raise TypeError(
                f"Cannot perform operation {op_name} between DateTimeNano and {type(value)}"
            )
    ...
    def random(cls, sz, to_tz="NYC", from_tz="NYC", inv=None, start=None, end=None):
        ...
        """
        return cls._random(
            sz, to_tz=to_tz, from_tz=from_tz, inv=inv, start=start, end=end
        )
    ...
    def resample(self, rule, dropna=False):
        ...
        def get_time_unit(unit):
            if unit in TimeSpan.unit_convert_factors:
                ...
                raise NotImplementedError(
                    f"Currently supports frequency strings {[*self.FrequencyStrings]}"
                )
            return unit
...
class TimeSpanBase:
    ...
    def strftime(self, format, dtype="U"):
        ...
        if isinstance(self, np.ndarray):
            ...
            result = dt.strftime(
                dt.utcfromtimestamp(abs(self) / 1_000_000_000.0), format
            )
            if isnegative:
                # check dtype 'S'
                if dtype == "S":
...
class DateTimeNanoScalar(np.int64, DateTimeCommon, TimeStampBase):
    ...
    def repeat(self, repeats, axis=None):
        return DateTimeNano(
            self._np.repeat(repeats, axis=axis),
            to_tz=self._timezone._to_tz,
